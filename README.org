...
- [[#about][about]]
  ...
  - [[#note-on-naming-and-libraries][note on naming and libraries]]
- [[#bootstrap][bootstrap]]
  - [[#helpers][helpers]]
...
(defun use-package-handler/:alias (name _ value rest state)
  ""
  `(,@(mapcar #'use-package:autoload (mapcar #'car value))
    ,@(mapcar (-lambda ((sym . def)) `(defalias ',sym #',def))
	      value)
    ,@(use-package-process-keywords name rest state)))
#+end_src
...
[[][straight]]. :stuck-out-tongue: The reason I define my own is I will likely
...
(defun use-package-handler/:nameless (name _ value rest state)
  `((after! nameless ,@(mapcar (fn! `(push ',<> nameless-global-aliases)) value))
    ,@(use-package-process-keywords name rest state)))
#+end_src
...
\(fn ACTION (WHERE &optional ARGS-LIST TARGET &rest TARGETS) &rest BODY)"
  (-let* ((symbols-only (lambda (it) (and (symbolp it) (not (keywordp it)))))
    ...
    `(progn
       ,@(-map (fn `(,advice-macro ,name (,@before ,<> ,@after) ,@body))
               fns))))
#+end_src
...
(use-feature! em-term
  :idle-require t
  :config
  (mapc (fn! (pushnew <> eshell-visual-commands))
