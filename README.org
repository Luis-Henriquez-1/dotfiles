...
initialization of the frame and it avoids the usage of. Only problem is this
will cause your emacs to crash if the font you specify does not exist. And, you
can't check the existing fonts at this time either because the [[][frame hasn't
been initialized yet]] so functions like [] and [] will not return reliable
values (trust me, I tried).
...
This is the heart of my emacs config. It contains the library.
...
#+begin_src emacs-lisp
(xl-dir! XL-HOME-DIR (expand-file-name "~/"))

(xl-dir! XL-MULTIMEDIA-DIR (xl-home-dir "Multimedia/")
  "Directory used to store multimedia.")

(xl-dir! XL-SCREENSHOT-DIR (xl-multimedia-dir "screenshots/")
...
(xl-dir! XL-DOCUMENTS-DIR (xl-home-dir "Documents/")
  "Directory where documents are stored.")

(xl-dir! XL-ORG-DIR (xl-documents-dir "org/")
  "Directory where org files should go.")

(xl-dir! XL-DOWNLOADS-DIR (xl-home-dir "Downloads/")
  "Directory where downloads are stored.")

(xl-dir! XL-TRASH-DIR (xl-home-dir "Trash/")
  "Directory where downloads are stored.")
#+end_src
...
***** helm org tags
:PROPERTIES:
:ID:       c4109ac4-1754-4159-b167-7a17b5945240
:END:

The purpose of this function is to make adding tags to headlines easy and
painless.

****** tag history
:PROPERTIES:
:ID:       398cb7dd-9992-47f0-b692-dbb3b1fa6093
:END:

#+begin_src emacs-lisp
(defvar helm-org:tag-history nil
  "History for helm tags.")
#+end_src

****** sources
:PROPERTIES:
:ID:       824657eb-930b-430d-b76b-d370871cebd0
:END:

******* main source
:PROPERTIES:
:ID:       c738b06f-0978-48e5-a396-426d8d927810
:END:

#+begin_src emacs-lisp
(defun helm-org:set-tags-source ()
  (helm-build-sync-source "Select Tag(s): "
    :candidates (-uniq (-flatten (org-map-entries #'org-get-tags nil 'file)))
    :action helm-org:tag-actions))
#+end_src

******* tag is not found
:PROPERTIES:
:ID:       52237aea-d5ff-41dd-acac-0d523b66a7e6
:END:

The function [[helpfn:helm-mini][helm-mini]] shows how gracefully helm can deal with a candidate that
was not matched.

#+begin_src emacs-lisp
(defun helm-org:tag-not-found-source ()
  (helm-build-dummy-source "Create a new tag"
    :action (helm-make-actions "new tag" #'org-set-tags)))
#+end_src

****** actions
:PROPERTIES:
:ID:       05e8dca6-a563-4608-8a22-d96b9041991c
:END:

******* action variable
:PROPERTIES:
:ID:       89852f59-d81c-4676-92d2-2c40f74403da
:END:

This is the variable that stors the actions for this tag function.

#+begin_src emacs-lisp
(defvar helm-org:tag-actions nil
  "List of actions for org tags.")
#+end_src

******* dwim set tags
:PROPERTIES:
:ID:       343b502b-c4f9-4bfd-9ed7-7d9a097c0310
:END:

#+begin_src emacs-lisp
(push '("dwim tags" . helm-org:dwim-set-tags-action) helm-org:tag-actions)

(defun helm-org:dwim-set-tags-action (tag)
  "Set the tag."
  (let ((selected (or (with-helm-buffer (helm-marked-candidates)) (list tag)))
	(current (org-get-tags)))
    (alet (append (-difference current selected)
...
******* set tags
:PROPERTIES:
:ID:       351a0e4d-3f78-4913-b512-5a1c8c5710a3
:END:

#+begin_src emacs-lisp
(push '("set tags" . helm-org:set-tags-action) helm-org:tag-actions)

(defun helm-org:set-tags-action (tag)
  "Set tags to choosen tags."
  (alet (or (with-helm-buffer (helm-marked-candidates)) (list tag))
    (org-set-tags selected)))
#+end_src

******* remove tags
:PROPERTIES:
:ID:       2e4fe319-3f26-48ff-a729-ed7c6f5dbefe
:END:

#+begin_src emacs-lisp
(push '("remove tags" . helm-org:remove-tags-action) helm-org:tag-actions)

(defun helm-org:remove-tags-action (tag)
  "Set the tag."
  (let ((selected (or (with-helm-buffer (helm-marked-candidates)) (list tag)))
	(current (org-get-tags)))
    (alet (-difference current selected)
      (org-set-tags it))))
#+end_src

****** main function
:PROPERTIES:
:ID:       ae790418-8028-4989-9f81-835188184849
:END:

#+begin_src emacs-lisp
(defun helm-org:set-tags ()
  "Set tags for headline."
  (interactive)
  (helm :sources (helm-org:set-tags-source)
	:buffer "*helm tags*"
	:history helm-org:tag-history))
#+end_src

****** binding
:PROPERTIES:
:ID:       13eff3e2-f04b-493e-911e-8f641b36e91f
:END:

Here I make sure that.

#+begin_src emacs-lisp
(general-def [remap org-set-tags-command] #'helm-org:set-tags)
#+end_src
...
******* generic template
:PROPERTIES:
:ID:       3e8e8230-1732-48f8-8ec1-47333e0aea2d
:END:
...
******* todo
:PROPERTIES:
:ID:       ba9efcea-c600-4304-8b0a-f478eb84a3aa
:END:
...
******* website
:PROPERTIES:
:ID:       c669df23-de45-4567-83b7-03534877c865
:END:

The purpose of this capture template is to take a "webshot" of the current
webpage. Let me explain. The website is a source of extremely valuable
information. Almost always when I have a question I peruse duckduckgo or (if
that fails) google. The problem is that typically I'll need to look at a
solution I find again. It can take a long time to find the exact place. What's
worse, I have no guarantee of finding the same information again. Over time the
content of a webpage may change or the webpage itself may be taken down
entirely. That's why I want to collect offline copies of webpages that are
useful to me.

******** webshot directory
:PROPERTIES:
:ID:       d63904d7-0e65-4a75-b7a7-e68065811208
:END:

#+begin_src emacs-lisp
(xl-dir! XL-WEBSHOT-DIR (xl-multimedia-dir "webshots")
  "Directory where webshots should go.")
#+end_src

******** use exwm to get the link of the website
:PROPERTIES:
:ID:       ecba4a3d-0926-4062-a5e7-34e7c2fceb36
:END:

#+begin_src emacs-lisp
#+end_src

******** use weasyprint to take webshot
:PROPERTIES:
:ID:       f5f98244-da1a-4530-a6f9-83b07786c12f
:END:

#+begin_src emacs-lisp
(defun xl-take-webshot (url)
  "Take a webshot of URL."
  (let ((default-directory ()))
    (call-process "weasyprint" nil nil nil url)))
#+end_src

******** website capture string
:PROPERTIES:
:ID:       02ba52bd-9de7-4e5d-8f48-a961f2e0e5e8
:END:

#+begin_src emacs-lisp
(defun org-capture:website-template ()
  "Return emacs capture template."
  (->> (org-ml-build-headline! :title-text "%?")
       (org-ml-headline-set-node-property "ID" (org-id-new))
       (org-ml-headline-set-node-property "CREATED" (ts-format))
       (org-ml-headline-set-node-property "LINK" )
       (org-ml-to-trimmed-string)))
#+end_src

******** template
:PROPERTIES:
:ID:       c96be217-04ef-4602-a78b-fb842cde6eee
:END:
