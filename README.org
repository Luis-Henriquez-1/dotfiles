...
=nowebs= are what I mean when I say "dynamic" generation. It allows me to add
the result of a lisp expression that was computed when the org file was tangled.

One way you could insert the evaluated code is by using <<noweb()>>.

Although nowebs are nice, I do find them lacking.
...
* yadm
:PROPERTIES:
:ID:       02d273d2-9d00-437f-91c2-c783da9f2b5b
:END:

=yadm= is a dotfile manager. It's mantra is that "if you know git, you know
yadm". What I like about it is that it's easy to pickup since it really does
closely follow git; and that it stays out of your way. Other dotfile managers
change the names of your files and symlink them. Although it can be nice to
remove the =.= from dotfiles in an attempt to make them more readable, it is
also adds another layer of complexity. Yadm requires no symlinks and it keeps
your files the same name.

** install yadm
:PROPERTIES:
:ID:       631b8a3a-85bb-4a31-a962-0a8809e26682
:END:

#+begin_src emacs-lisp
(shell-command "https://github.com/TheLoce/.yadm-project")
#+end_src

** add yadm to load-path
:PROPERTIES:
:ID:       5f700bc6-74ac-4657-a37d-ef52c92ef8e2
:END:
...
** config.scm
:PROPERTIES:
:ID:       1842dbb7-0b15-4ab8-ae68-1e420badd400
:END:

The =config.scm= is the file that determines the configuration of the entier
Guix operating system.

*** miscellaneous
:PROPERTIES:
:ID:       9cbaa260-6072-4734-bd04-e3f8959058ac
:END:
...
:PROPERTIES:
:ID:       e87fb064-1827-4b8c-b6c5-4bd1deff3d91
:END:
...
*** user accounts
:PROPERTIES:
:ID:       bed74fa7-9238-4dcf-8d3e-a5a8d8e81832
:END:
...
*** services
:PROPERTIES:
:ID:       9d8608e3-acbf-420e-b37a-a8da1496bfdc
:END:

**** elogind service
:PROPERTIES:
:ID:       75b86c4a-e1f6-4060-89e4-ffc052a924f2
:END:

By default =elogind= does not hibernate or suspend the computer when the lid is
closed if that computer is connected to a powersource. In my experience, laptops
I've seen suspend when their lid is closed; and personally, I prefer this.
...
*** packages
:PROPERTIES:
:ID:       85a7f8ca-f29f-4e2d-9670-d3331be986b2
:END:
...
*** full operating system declaration
:PROPERTIES:
:ID:       375877df-54ef-4f8c-8eff-0f27e2f1fb14
:END:
...
#+begin_src conf :tangle ~/test.txt :noweb tangle
[user]
...
* xdg specification
:PROPERTIES:
:ID:       3800bbf1-1bb6-427f-a339-eb793a75a401
:END:

The xdg specification is a standard for how dotfiles required by a system
package should be stored. It says that dotfiles should be stored in a path of
the form =~/.config/PACKAGE/=, where PACKAGE is a directory whose name is the
name of the package. So, for example, under the XDG specification emacs dotfiles
are stored in =~/.config/emacs/=.

This XDG specification hasn't always existed so there are several system
packages that write dotfiles in other places. This headline is where I declare
bash variables that specify where these directories should go.

** xdg data and config
:PROPERTIES:
:ID:       50606e70-a834-4207-b5e9-4f53e3c267e3
:END:

#+begin_src emacs-lisp
;; the "data" refers to files.
(eshell/export "XDG-DATA-HOME" (expand-file-name "~/.local/share"))

;; the "config" refers to configuration files.
(eshell/export "XDG-CONFIG-HOME" (expand-file-name "~/.local/share"))
#+end_src

** editors
:PROPERTIES:
:ID:       81752047-43cc-481f-9fe0-7ae66124e9fa
:END:

#+begin_src emacs-lisp
;; Default Programs.
(eshell/export "EDITOR" "emacs")
(eshell/export "TERMINAL" "xterm")
(eshell/export "BROWSER" "icecat")
#+end_src

** gpg
:PROPERTIES:
:ID:       404d28db-4e9a-4e87-a1cb-92c46bd80363
:END:

#+begin_src emacs-lisp
;; Directories.
(eshell/export "GNUPGHOME" (expand-file-name "gnupg" xdg-data-home))
(eshell/export "lesshistfile" "-")
#+end_src
...
(defvar xl-backup-fonts (list "hermit" "mononoki" "iosevka")
  "List of fonts that should be used.")

(defun xl-most-preferred-font-available ()
  "Return the most prefered font that's available."
  (cl-first (-any-p (string-match-p it) xl-backup-fonts) xl-backup-fonts))

(xl-most-preferred-font-available)
#+end_src
...
to set up [[][xresources]]. Prevent the glimpse of un-styled Emacs by disabling
...
(xl-dir! XL-SCREENSHOT-DIR (xl-emacs-dir "screenshots/")
