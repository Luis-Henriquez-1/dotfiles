...
#+begin_src emacs-lisp
(use-package:dwim-insert :xl-silence)

(defun use-package-normalize/:xl-silence (_ _ args)
  (cl-assert (-all-p #'symbolp args))
  args)

(defun use-package-handler/:xl-silence (name _ value rest state)
...
#+begin_src emacs-lisp
(use-package:dwim-insert :xl-gc-pause)

(defun use-package-normalize/:xl-gc-pause (_ _ args)
  (cl-assert (-all-p #'symbolp args))
  args)

(defun use-package-handler/:xl-gc-pause (name _ value rest state)
...
#+begin_src emacs-lisp
(use-package:dwim-insert :xl-ignore)

(defun use-package-normalize/:xl-ignore (_ _ args)
  (cl-assert (-all-p #'symbolp args))
  args)

(defun use-package-handler/:xl-ignore (name _ value rest state)
  `(,@(-map (-partial #'use-package:autoload name) value)
...
#+begin_src emacs-lisp
(use-package:dwim-insert :xl-always-yes)

(defun use-package-normalize/:xl-always-yes ()
  (cl-assert (-all-p #'symbolp args))
  args)

(defun use-package-handler/:xl-always-yes (name _ value rest state)
  `(,@(-map (-partial #'use-package:autoload name) value)
    ,@(--map `(xl-always-yes #',it) value)
    ,@(use-package-process-keywords name rest state)))
#+end_src

******** log
:PROPERTIES:
:ID:       46b23feb-eec6-45e6-b6c5-91eeda400f4b
:END:
...
(defun use-package-handler/:xl-hook (name _ value rest state)
  "Expect an alist of (HOOK FN)."
  `(,@(-map (-lambda ((hook . fn)) `(xl-hook-add ',hook ',fn)) value)
...
#+begin_src emacs-lisp
(use-package:dwim-insert :xl-before-call)

(defun use-package/:xl-before-call (name key args)
  (cl-assert (-all-p (-orfn #'symbolp #'listp) args))
  (->> args
       (--map-when (symbolp it) (cons name it))
       (-mapcat #'use-package:normalize-cons)))

(defun use-package-handler/:xl-before-call (name key value rest state)
...
#+begin_src emacs-lisp
(use-package:dwim-insert :xl-after-call)

(defalias 'use-package-normalize/:xl-after-call
  #'use-package-normalize/:xl-before-call)

(defun use-package-handler/:xl-after-call (name _ value rest state)
  `(,@(-map (-partial #'use-package:autoload name)
	    (-map #'cdr value))
...
#+begin_src emacs-lisp
(use-package:dwim-insert :xl-set)

(defun use-package-normalize/:xl-set (_ _ args)
  (cl-assert (-all-p #'listp args))
  args)

(defun use-package-handler/:xl-set (name _ value rest state)
...
This keyword integrates with [[id:b3351a74-b2d0-42b8-9895-b6baad0adb0b][dbc]] (display buffer control).

#+begin_src emacs-lisp
(use-package:dwim-insert :xl-rule)

(defun use-package-normalize/:xl-rule (_ _ args)
  (cl-assert (-all-p #'xl-true-list-p args))
  args)

(defun use-package-handler/:xl-rule (name _ value rest state)
...
This keyword integrates with [[0d619336-e852-4c6a-89a8-38ccbb71a077][idle-require]].

#+begin_src emacs-lisp
(use-package:dwim-insert :xl-idle-require)

(defun use-package-normalize/:xl-idle-require (name _ args)
  (cl-assert (-all-p #'symbolp args))
  (--map-when (eq t it) name args))

(defun use-package-handler/:xl-idle-require (name _ value rest state)
...
#+begin_src emacs-lisp
(use-package:dwim-insert :xl-alias)

(defun use-package-normalize/:xl-alias (_ _ args)
  args)

(defun use-package-handler/:xl-alias (name _ value rest state)
...
#+begin_src emacs-lisp
(use-package:dwim-insert :xl-ensure)
...
#+begin_src emacs-lisp
(use-package:dwim-insert :xl-namless)

(defun use-package-normalize/:xl-nameless ()
...
(defun use-package-handler/:xl-nameless ()
...
#+begin_src emacs-lisp
(defconst XL-ADVICE-REGEXP (rx (group (1+ (not white))) "@" (group (1+ (not white))))
...
(defun xl-advice-add (symbol where fn &optional props expire-fn)
  ...
  (let ((advice (xl-advice-name symbol fn)))
    ...
    (when expire-fn
      (alet (xl-expire advice expire-fn)
...
(defun xl-expire (fn &optional expire-condition)
  ...
  (alet (gensym "xl-expire-")
    (fset it (xl-gen-expire-advice expire-condition))
...
#+begin_src emacs-lisp
(defun xl-always-yes (fn &optional props expire-fn)
  "Ignore yes or no prompt."
  (xl-advice-add fn :around #'xl-always-yes-advice props expire-fn))

(defun xl-always-yes-advice (orig-fn &rest args)
  "An advice that makes `y-or-n-p' return t without prompt."
  (noflet ((y-or-n-p (prompt) t)) (apply orig-fn args)))
#+end_src
...
(defun xl-silence-output (fn &optional props expire-fn)
  ...
  (xl-advice-add fn :around #'xl-silence-output-advice))
#+end_src
...
log function that is added to FN."
  (xl-advice-add fn (if after :after :before) (xl-new-log-advice fn)))

(defun xl-new-log-advice (fn)
  "Return new log advice for FN."
  (alet (gensym (format "xl-log-%s-" fn))
...
#+begin_src emacs-lisp
(defun xl-pause-gc (fn &optional props expire-fn)
  "Pause garbage collection for the duration of FN."
  (xl-advice-add fn :around #'xl-pause-gc-advice props expire-fn))
...
(defun xl-ignore (fn &rest props expire-fn)
  "Ignore FN."
  (xl-advice-add fn :override #'ignore props expire-fn))
#+end_src
...
#+begin_src emacs-lisp
(--each (list #'consult-outline #'consult-line)
  (xl-advice-add it :after #'outline:show-branch-maybe))
#+end_src
...
#+begin_src emacs-lisp
(--each (list #'consult-outline #'consult-line)
  (xl-advice-add it :after #'outline:display-children-in-window))
#+end_src
...
(use-package! ace-jump-helm-line
  ...
  (ace-jump-helm-line-move-only-key  . ?r)
  (ace-jump-helm-line-persistent-key . ?s))
#+end_src

***** helm-posframe
:PROPERTIES:
:ID:       a24a4f6d-d3ed-4df8-a7e0-666cbdc873ff
:END:
...
(use-feature! ert
  ...
  :xl-always-yes ert-delete-all-tests)
#+end_src
...
(alet #'xl--load-and-initialize-exwm-maybe-advice
  (xl-advice-add #'browse-url :before it nil t))
...
#+begin_src emacs-lisp
(xl-hook-add 'org-edit-source-mode-hook #'delete-trailing-whitespace)
#+end_src
