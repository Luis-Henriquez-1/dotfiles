...
(defun use-package-normalize/:xl-before-call (name key args)
...
#+begin_src emacs-lisp
(use-package:dwim-insert :xl-nameless)

(defun use-package-normalize/:xl-nameless (package _ args)
...
(defun use-package-handler/:xl-nameless (name _ value rest state)
...
(defun xl-expire (fn &optional expire-condition)
  ...
  (let ((expire-fn (xl-symbol-intern fn '@expire))
	(expire-condition (if (booleanp expire-condition)
			      '(lambda () t)
			    expire-condition)))
    (fset expire-fn (xl-expire-lambda fn expire-condition))
...
***** helm-buffer
:PROPERTIES:
:ID:       db9440d4-7701-4120-b381-598572c186c6
:END:

#+begin_src emacs-lisp
(use-feature! helm-buffer)
#+end_src
...
This package downloads docsets from online.
...
#+begin_src emacs-lisp
(--each (list #'org/dwim-edit-source-block #'org/dwim-edit-description)
...
(define-leader-key!
  ...
  "s" (list :def #'swiper-helm                 :wk "jump to line")
  "w" (list :def #'engine/search-duckduckgo    :wk "web")
  "h" (list :def #'helm-org-in-buffer-headings :wk "jump to outline heading"))
#+end_src
