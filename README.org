...
(defun xl-always-yes (fn &optional props expire-fn)
  "Ignore yes or no prompt."
  (xl-add-advice fn :around #'xl-always-yes-advice props expire-fn))
...
log function that is added to FN."
  (xl-add-advice fn (if after :after :before) (xl-new-log-advice fn)))
...
(defun xl-message-with-xl-log (fn &rest props expire-fn)
  ...
  (xl-add-advice fn :around #'xl-message-xl-log-advice props expire-fn))
...
(defun xl--load-on-call (package where fn)
  ...
  (alet (xl-symbol-intern 'xl--load- package '-advice)
    (fset it `(lambda (&rest _)
                ...
                (require ',package)))
    (xl-add-advice fn where it nil t)))
#+end_src
