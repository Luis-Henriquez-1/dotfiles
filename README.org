...
- [[#about][about]]
  ...
  - [[#how-are-dotfiles-generated][how are dotfiles generated?]]
...
a usb containing my private gpg key and my dotfiles stored on github. By the
way, by completely I mean /completely/--I should be able to not only reproduce
the packages I have but their versions as well.
...
I've spent *alot* of time on Emacs. I feel an obligation to society. Not to get
to personal here, but I feel I am very fortunate to have learned so many
things. And I feel that I have not contributed enough to my community.
...
*** an *extremely* powerful emacs configuration featuring...
:PROPERTIES:
:ID:       bea8c3aa-8b2b-4269-ba6e-dcb037f32606
:END:

**** a rich set of custom use-package keywords
:PROPERTIES:
...
Because I'm using an org configuration.
...
**** fast candidate selection featuring selectrum
:PROPERTIES:
...
** how are dotfiles generated?
:PROPERTIES:
:ID:       e208e820-cb37-40f5-912e-bad4abf1fd5a
:END:

I haven't worked out yet how this is going to work. :confounded:

Some things to think about. Obviously, Emacs is the centerpiece of my
dotfiles. It's essentially an interface with which I interact with an underlying
OS.

One thing is I don't want to tangle everything all at once.
...
* bootstrap
:PROPERTIES:
:ID:       7b8fc24d-7e1e-4e11-bb9d-925867631993
:END:
...
[[https://github.com/okamsn/loopy][loopy]] and [[https://github.com/magnars/dash.el][dash]]. Additionally, I want to have some choice between
which dotfiles are tangled/generated and which are not. For sure I'm going to
want Emacs tangled no matter what but I might not want to tangle other dotfiles.
On a computer with Windows or Mac OS, for example, there's no point in tangling
the =.xinitrc= because I can't use Xorg.

Org does not have an =org-babel-tangle-subtree=, but it's possible to do
something [[https://lists.gnu.org/r/emacs-orgmode/2011-01/msg01173.html][as mentioned here]].
...
and my emacs is in an inconvenient to fix whatever problem I might have. The
reason I favor [[https://github.com/ch11ng/exwm][exwm]] over [[https://github.com/stumpwm/stumpwm][stumpwm]] is that the former is more tightly integrated
into Emacs. I haven't really configured stumpwm. Maybe I will in the future, but
I don't know--Emacs is really time consuming and I already use exwm anyway. Good
...
#+begin_src emacs-lisp :tangle no
(defun dotfiles:mbsyncrc-imap-account-setup (entry)
  "Return the account part of the."
  (with-pass-fields! netry
...
*** full config
:PROPERTIES:
:ID:       a040b172-63f1-42fb-b35c-22a6aba5dd5a
:END:
...
#+name: msmtprc-accounts
#+begin_src emacs-lisp :tangle no
(defun dotfiles:msmtprc-accounts ()
  (thread-last (dotfiles:email-accounts)
...
<<eval((dotfiles:msmtprc-accounts))>>
