...
programmers and linux users. I believe that Emacs.
...
completely reproducable--the other is [[][Nix OS]].
...
The original idea for this macro came from [[https://gitlab.common-lisp.net/iterate/iterate/-/blob/master/iterate.lisp][iter's source coe]].
...
Some variables in emacs have [[][custom setters]]. I don't want to have to
figure out which ones do and do not have these setters. This macro sets the
custom setter of there is any.
...
I got the idea of this macro from [[][Radian]]. It differentiates built-in
packages from external ones.
...
[[][straight]]. The reason I define my own. :P
...
Also, remember [[][I mentioned]] I want a fast startup time.
...
(use-package! selectrum
  ...
  ;; :xl-bind
  ;; (:state (insert emacs) :map selectrum-minibuffer-map)
...
=consult= is a package that's akin to =counsel=. It's a library.
