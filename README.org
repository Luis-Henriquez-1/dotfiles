...
check for are one and zero. It is often a useful distinction whether a list has
just one item or many for example. Emacs already has a function for checking is a
value is zero, [[helpfn:zerop][zerop]]. But it doesn't have one for one. So here we are.
...
You might wonder why I have this function instead of the similar [[helpfn:-const][-const]] provided
by dash. Dash's function returns a compiled function.
...
****** enable buffer rules
:PROPERTIES:
:ID:       b642c12d-ff3e-4ee8-9998-aea360e7ed7d
:END:

#+begin_src emacs-lisp
(defhook! enable-buffer-rules (emacs-startup-hook)
...
=elog= is a library for generating logging functions. I use [[helpfn:elog-open-log][elog-open-log]] to
generate a function that logs to the buffer =*xl-log*=.
