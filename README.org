...
(defconst XL-ANAPHORIC-SYMBOL-REGEXP
  (eval-when-compile (rx "<" (group (+ (not (any white ">" "<")))) ">"))
...
(use-package:override :xl-hook :hook)
...
***** defhookvar!
:PROPERTIES:
:ID:       98630924-d2c3-4c86-b326-381b93c82ecd
:END:

#+begin_src emacs-lisp
(defmacro defhookvar! (name where value docstring)
  "Create a hook variable after."
  ()
  (xl-advice-add ))
#+end_src
...
***** after-load-hook
:PROPERTIES:
:ID:       a0c6da0c-aca4-4b99-a785-02953c1f758e
:END:

In Emacs, the way you defer evaluation of a form until something is loaded is by
using [[helpfn:eval-after-load][eval-after-load]]. I use the [[id][after!]] macro I wrote which is just a wrapper
around =eval-after-load=. What I don't like about the =after!= macro is that it
stores the form. So what I decided to do is for each package I use, create a
=package:after-load-hook=. This hook will run. But what it will allow me to do
is have better control over forms I want evaluated after the loading of a
package. For instance, these forms will named hooks now. And, they'll be in a
variable.

#+begin_src emacs-lisp
(defun xl-create-after-load-hook (package)
  "Create an after load hook for package."
  (let ((name (xl-symbol-intern package ':after-load-hook))
	())
    `(unless (boundp ',name)
       (defvar ,name nil
	 ,(format "Hook run after %s is loaded." package))

       (if (featurep package)
	   nil
	 (eval-after-load package
	   '(run-hooks ',package:after-load-hook))))))
#+end_src

***** anonymous functions
:PROPERTIES:
:ID:       2baa3c3a-45f6-4eb1-94d8-c2ef9ff3bd23
:END:
...
This is a macro for writing anaphoric lambdas. It is based on [[][fn!]], the
main difference being that in a few special cases you can specify a named
anaphoric variable. Also, I don't support the alternate =fn:= syntax.
...
are :pouting_cat:. It also means it's more difficult to incorporate it with the
...
******* alist of default
:PROPERTIES:
:ID:       db39c795-33f0-4a3c-a5a4-06f2b873c254
:END:

#+begin_src emacs-lisp
(defvar mu4e-context:default-vars
  `((mu4e-sent-folder      . ,(+mu4e-guess-folder base-dir "sent"))
...
:PROPERTIES:
:ID:       f2ce41fa-ee55-4d37-9023-6511e393666b
:END:
...
(defun org-edit-paragraph:maybe-remove-newline ()
  ...
  (when (and (-all-p (fn! (= 10 <>)) (buffer-string))
...
Modal editing is widely accepted to be more efficient than modeless editing. The
most popular modal. There are several packages.

As much as I hate to say it, to really become efficient at editing you have to
invest time to fine-tuning your editing commands at a fine-grained level. You
need to objectively analize you're editing and find things that could be
improved.

***** modalka
:PROPERTIES:
:ID:       fccbf279-f4c4-4997-ac8d-acc31b2276dd
:END:

=modalka= is a modal editing framework.

The idea I belive is to make modalka normal state and treat the absence of
modalka (essential default Emacs) as insert state.

I started out as a vim user and then I moved to evil. However, as I begin to
become a more advanced Emacs user and I looked at projects like [[][selectrum]]
which try to take the "path of least resistance", I started to realize the value
of aspiring to take advantage of already existing frameworks. Hate it or love
it, Emacs comes with key-chord heavy bindings by default. And they're not
random--[[][there is a logic behind them]]. What I want is to use them.

It's easy to want to abandon work done on something and just do your own
thing. It seems easier to create something than to understand and compromise an
existing thing by authors that are not you. But often it is not. It is unwise to
discredit the conscious thought and effort of others who have earnestly
attempted to solve a problem without a very good reason and without trying to
...
****** init
:PROPERTIES:
:ID:       96ceb340-30f9-4228-93ea-45dda153b2df
:END:
...
****** space bindings
:PROPERTIES:
:ID:       b311f414-67c3-451a-8cda-9a19388bda6e
:END:

#+begin_src emacs-lisp
(modalka-define-kbd "W" "C-x")
...
****** create a space prefix
:PROPERTIES:
:ID:       714b36d2-8d4a-490d-bec6-dc75fe3a1a54
:END:

***** ryo-modal
:PROPERTIES:
:ID:       3c5bc81c-2b14-448c-86c4-b19401795236
:END:

=ryo-modal= provides a modal editing framework to.

***** meow
:PROPERTIES:
:ID:       9353943c-c6a9-480b-a22b-9d1c0e0f48e3
:END:

=meow= stands for "Modal Editing On Wish". It's a package that provides its own
[[][Kakourne]] like modal-editing scheme. Keep in mind that the distinguishing
feature of Kakourne is that it combines [[][Vim's]] normal mode and visual mode
into one. The idea is that you can interactively see what's selected before
changing operating on that region.

The main idea of meow is that to provide modal editing while taking advantage of
as much existing Emacs bindings.

What attracts me to meow is that it seems to be very well thought out.

****** init
:PROPERTIES:
:ID:       5c7996a5-319b-4e9e-b28e-95b8b2e5cd0a
:END:
...
****** bindings
:PROPERTIES:
:ID:       9dea4b44-bece-416a-b338-ff42774cb17e
:END:

******* digit arguments
:PROPERTIES:
:ID:       af748fec-698c-49bb-9c1d-dd28e4d4a087
:END:

#+begin_src emacs-lisp
(defun meow:setup-digit-arg-bindings (args)
  "Set up digit argument bindings."
...
******* ensure j and k are used for up and down movements
:PROPERTIES:
:ID:       7e91ed37-8046-47f2-ac37-178eb55be949
:END:

#+begin_src emacs-lisp
(defun meow:setup-j-and-k-bindings (args)
  "Set up j and k bindings."
...
******* normal bindings
:PROPERTIES:
:ID:       ff4a7714-4ef9-4344-a125-ec477aac84d4
:END:
...
******* setup all bindings
:PROPERTIES:
:ID:       3477d154-4a00-4106-8ed6-1b9710fc5d60
:END:

#+begin_src emacs-lisp
(defun meow:setup-bindings ()
  "docstring"
  (meow:setup-digit-arguments)
  (meow:setup-j-and-k-everywhere)
  ())
#+end_src

****** meow setup
:PROPERTIES:
:ID:       240bb38e-073e-4294-88d7-36696c2e67b8
:END:
...
**** themes
:PROPERTIES:
:ID:       62d543c2-19be-4483-bb1b-0995e84b57a2
:END:

Emacs theme changes the colors Emacs uses for display.

***** use-theme!
:PROPERTIES:
:ID:       6b695326-ab06-4f9f-baf4-a8e1f30dbd14
:END:

#+begin_src emacs-lisp
(use-package! use-theme
  :ensure (:host github :type git :repo "canatella/use-theme"))
(require 'use-theme)
(use-theme spacemacs)
#+end_src
...
***** random-thme
:PROPERTIES:
:ID:       34705923-f5c6-4852-bd17-cbd8c9ce1ad9
:END:

***** theme-looper
:PROPERTIES:
:ID:       376fe082-cc91-468b-8a20-a3e299c7fd5f
:END:
