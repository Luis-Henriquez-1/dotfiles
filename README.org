...
(advice-add #'use-package-normalize/:ensure :override #'use-package-normalize/:xl-ensure)
(advice-add #'use-package-handler/:ensure :override #'use-package-handler/:xl-ensure)
#+end_src
...
enough that I consider it worth it to give it its own keyword.
...
***** new popup rule just for helm
:PROPERTIES:
:ID:       1923fbb0-b68b-4ca6-8eaf-34276ea3c811
:END:

#+begin_src emacs-lisp
(alet '(display-buffer-in-side-window . ((side . bottom) (window-height . 0.6)))
  (dbc-add-ruleset "helm" it))
#+end_src
...
(use-package! helm
  :commands helm-M-x
  :hook emacs-startup-hook
  :ignore helm-display-mode-line
  :popup ("helm" "helm" :newname "\\*[Hh]elm.+")
  :set
...
(use-package! helm-org
  :before-call (org-capture . helm-org-capture-templates)
  :init
...
***** helm-swoop
:PROPERTIES:
:ID:       87c958ef-4834-4514-9e31-bdd21cbad98f
:END:

=helm-swoop= is like =swiper-helm= but has a lot more features.
...
(use-package! org-source
  :popup ("bottom" "org source blocks" :newname "\\*Org[[:space:]]\\(?:S\\(?:ource\\|rc\\)\\).+")
  :ensure nil)
#+end_src
...
(use-feature! org-refile
  :idle-require t
  :set
  (org-refile-targets                     . (list (xl-capture-file)))
