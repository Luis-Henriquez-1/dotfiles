...
(defun use-package-handler/:xl-after-call (name key value rest state)
  `(,@(-map (-partial #'use-package:autoload name)
	    (-map #'cdr value))
...
(defun use-package-normalize/:xl-rule (name key args)
  (cl-assert (-all-p #'xl-true-list-p args))
  args)

(defun use-package-handler/:xl-rule (name key value rest state)
...
(defun use-package-normalize/:xl-ensure ()
