...
#+begin_src emacs-lisp
(use-package:dwim-insert :xl-assume-yes)

(defun use-package-normalize/:xl-assume-yes (_ _ args)
  (cl-assert (-all-p #'symbolp args))
  args)

(defun use-package-handler/:xl-assume-yes (name _ value rest state)
  `(,@(-map (-partial #'use-package:autoload name) value)
...
****** internal advice name
:PROPERTIES:
:ID:       9ce4d727-7400-4b3e-83b8-0177005f2141
:END:

#+begin_src emacs-lisp
(defun xl-internal-advice-name (advice)
  (nth 1 (s-match "xl--\\(.+\\)-advice" (symbol-name advice))))
#+end_src
...
(defun xl-advice-name (fn advice)
  ...
  (alet (or (xl-internal-advice-name advice) advice)
...
(defun xl-expire (fn &optional expire-condition)
  ...
  (alet (xl-symbol-intern fn '@expire)
    (fset it (xl-gen-expire-advice expire-condition))
    (advice-add fn :around it)
    (xl-log-function it)))
#+end_src
...
(use-feature! ert
  ...
  :xl-assume-yes ert-delete-all-tests)
#+end_src
