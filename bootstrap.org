...
(defun iteratively-get-depth (recipe)
  ...
  (iter (with (current-depth (shell-command (format "cd %s && git log --oneline | wc -l"))))
	(expr current-commit (straight-vc-get-commit "git"))
	(until (string= commit current-commit))
	(shell-command (format "cd %s && git fetch --depth=$%s" (cl-incf current-depth)))))
#+end_src
...
(defun xl-straight-repos-lacking-depth (recipes)
  ...
  (iter (for (recipe &as package . plist) recipes)
	...
	;; (message "%s vs %s" initial-commit recipe-commit)
