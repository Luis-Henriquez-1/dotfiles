...
(defun dotfiles:msmtprc-account-string (entry)
  ...
  (let* (($account (password-store-get-field entry "account"))
    ...
    (conf:convert-to-string
     ...
     :port $out-port
     :user $email
     :passwordeval (conf:string "pass %s" entry))))
#+end_src
...
#+begin_src emacs-lisp :tangle no
(alet (-map #'dotfiles:msmtprc-account-string
...
(defun dotfiles:mbsyncrc-sync-gmail ()
  ...
  (let* ((mailboxes '("All Mail" "Drafts" "Sent Mail" "Spam" "Starred" "Trash"))
    ...
    (mapcar (lambda (name mailbox)
              (conf:convert-to-string
               :Channel (format "gmail-%s" name)
               :Master (format ":gmail-remote:\"[Gmail]/%s\"" name mailbox)
               :Slave (format ":gmail-local:" ())
               :Create 'Slave
               :SyncState '*))
            (-zip #'list mailboxes abbrevs))
    ((format "Channel gmail-%s\n"))
    ))
#+end_src
...
#+name:mbsync-setup
#+begin_src emacs-lisp
(alet (-map #'dotfiles:mbsyncrc-account-string
            (--filter (s-matches-p (rx bos "email/") it) (password-store-list)))
  (s-join "\n\n" it))
#+end_src
