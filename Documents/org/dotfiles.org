...
(defun dotfiles:mbsyncrc-sync-gmail (entry)
  (let* ((mailboxes '("All Mail" "Drafts" "Sent Mail" "Spam" "Starred" "Trash"))
         (names (s-downcase (-first-item (s-split-words it))) mailboxes))
    (alet (append (-map (dotfiles:mbsyncrc-sync-gmail-section) (-zip #'list mailboxes names))
                  (list "\nGroup gmail\n")
                  (--map (format ":Channel gmail-%s\n" it) names))
      (s-split "\n" it))))
#+end_src
