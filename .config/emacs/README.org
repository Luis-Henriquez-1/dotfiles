...
*** TODO straight keyword                                             :disabled:
...
(push `((or (memq leaf--key '(:straight))
        ...
        (if (ignore-errors (keywordp (caar leaf--value)))
            (list (cons leaf--name (car leaf--value)))
          leaf--value))
      leaf-normalize)
*** TODO =:on-hook= keyword                                             :disabled:

This keyword is for loading packages when a hook is first run.

#+begin_src emacs-lisp
(defun leaf::load-,package ()
  "An internal leaf function that loads"
  (require ',package)
  (remove-hook ',hook #'leaf::load-,package))

(leaf-insert-list-after)
#+end_src
