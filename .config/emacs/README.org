...
This package manages remembering.

#+begin_src emacs-lisp
(use-package! selectrum-prescient
  :init
  (after! selectrum (selectrum-prescient-mode)))
#+end_src
...
(defun selectrum/swiper ()
  ...
  (let* ((selectrum-should-sort-p nil)
         ;; (minimim-line-number (line-number-at-pos (point-min) t))
...
(defun selectrum/switch-buffer ()
  ...
  (let ((selectrum-should-sort-p nil)
        ...
        (recentf-paths recentf-list))
    (alet (completing-read "Select buffer: " (append other-buffers recentf-paths))
      (if (-contains-p recentf-list it)
          (find-file it)
        (switch-to-buffer it)))))

(define-key! [remap switch-to-buffer] #'selectrum/switch-buffer)
#+end_src
...
(defun selectrum/org-select-tags ()
  ...
  (let* ((current (org-get-tags (point)))
         (selected (completing-read-multiple "Select org tag(s): " #'org-tags-completion-function)))
    (alet (-uniq (append (-difference current selected)
                         (-difference selected current)))
      (org-set-tags it))))

(define-key! [remap org-set-tags-command] #'selectrum/org-select-tags)
#+end_src
...
(define-key! '(insert emacs) selectrum-minibuffer-map "C-a" #'embark-act)
