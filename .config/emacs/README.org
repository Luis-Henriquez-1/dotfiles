...
*** record-load-path-change!

#+begin_src emacs-lisp
(defmacro record-load-path-change! (&rest body)
  "Return new items added to load-path in BODY.
Preserve the original load path."
  (let ((old-load-path (make-symbol "old-load-path"))
...
#+begin_src emacs-lisp
(cl-defun leaf:define-keyword (&key name where target normalizer defer alias form)
...
(leaf:define-keyword
 ...
 '`(,@(when (and leaf--value (car-safe leaf--value))
        (mapcar (lambda (it) `(push ,it load-path))
                (record-load-path-change!
                 (unless after-init-time (straight:install-package (car leaf--value)))
                 ;; Not all the load-paths are added during macro expansion
                 ;; time. I don't yet know why. Until then, this seems to work.
                 (ignore-errors
                   (straight--add-package-to-load-path
                    (gethash (symbol-name leaf--name) straight--recipe-cache))))))
    ,@leaf--body))
