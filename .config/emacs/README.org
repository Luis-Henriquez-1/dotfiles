...
(defun org:dwim-description-bounds (point elements)
  ...
  (if (-none-p #'org:element-paragraph-p elements)
      ;; Is there a source block?
      ;; Skip all property drawers.
      nil
    ;; There is at least one paragraph element.
    (alet (->> elements
               (-partition-by #'org:element-paragraph-p)
               (--remove (not (org:element-paragraph-p (-first-item it))))
               (-first-item))
      (cons (org-element-property :contents-begin (-first-item it))
            (org-element-property :contents-end (-last-item it)))))
  ;; (acond
  ...
  ;;  ((-first (fn! (eq 'property-drawer (org-element-type <element>)))
  ;;           (reverse elements))
