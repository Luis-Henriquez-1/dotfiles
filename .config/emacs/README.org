...
STRUCTURE can be an alist, a plist, or a hash table."
  (declare (indent 1))
  (let* ((symbols (seq-filter (lambda (it) (and (symbolp it) (string-prefix-p "$" (symbol-name it))))
                              (-flatten body)))
         (keywords (seq-filter (lambda (it) (void:symbol-intern ":" (-second-item (s-match (rx "$" (group (* anything)))
                                                                                           (symbol-name it)))))
                               symbols)))
