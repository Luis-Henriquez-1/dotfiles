...
#+begin_src emacs-lisp
(dolist (package '(leaf-keywords leaf dash))
...
is the form that will be added to."
  ...
  (-let [(where place) where]
    (+leaf-insert-keyword name where place form)))
#+end_src
...
**** make leaf defer default                                          :disabled:
:PROPERTIES:
...
Bare symbols in BINDINGS are equivalent to:
  ...
  (let* ((bind-fn (lambda (bind)
         ...
         (symbols (-map (lambda (name) (make-symbol (symbol-name name))) names)))
    `(with-symbols! ,symbols
       (list 'let
             (-zip #'list (list ,@symbols) (list ,@forms))
             ,(cl-list* 'let
                        (-zip #'list names symbols)
                        body)))))
#+end_src
...
(defmacro let! (bindings &rest body)
  ...
  (let ((var-to-new (mapcar (lambda (var) (cons var (make-symbol (symbol-name var))))
                            (-uniq (flatten-list (mapcar #'car bindings))))))
    `(let!! ,(append
              (-map (lambda (it) (cons (cl-sublis var-to-new (car it))
                                       (cdr it)))
                    bindings)
              (-map (lambda (it) (list (car it) (cdr it)))
                    var-to-new))
       ,@body)))
#+end_src
...
(defmacro if-let! (bindings then &rest else)
  ...
  `(let! ,bindings
     (if (and ,@(flatten-list (-map #'car bindings)))
         ,then
       ,@else)))
...
(setq-default mode-linek-default-help-echo nil)
...
(defun selectrum/swiper ()
  ...
  (let* ((selectrum-should-sort-p nil)
         ...
         (line-choices (let ((minimum-line-number (line-number-at-pos (point-min) t))
                         ...
                         (-map-indexed (lambda (it it-index) (format "%d\t%s" it-index it)) buffer-text-lines)))
...
ELEMENTS is a list of headline elements (see `org:element-headline-elements')."
  (awhen! (-find (fn! (seq-some-p (apply-partially #'org:element-contains-point-p point)
                                  <it>))
