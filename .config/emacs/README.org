...
#+begin_src emacs-lisp
(defun void:anaphoric-body-symbols (body)
...
BODY contains any number of `<NAME>' where NAME refers to the name of the symbol."
  ...
  (let* ((symbols (thread-last (flatten-list body)
    ...
    `(let (,@(-map (-lambda ((sym name)) `(,sym (funcall ,fn ,structure ',name)))
...
*** TODO setup mu4e and download mail if it hasn't been
:PROPERTIES:
...
(defun mu4e/init ()
  ...
  (let ((email-dirs (--map (concat VOID:EMAIL-DIR it) (pass:email-list))))
    (unless (-all-p #'f-exists-p email-dirs)
      (message "creating directories that don't exist.")
      (--each email-dirs (mkdir it t))
      (mu4e-update-mail-and-index t)))
  (unless mu4e-contexts (mu4e:initialize-all-contexts)))
#+end_src
...
#+begin_src emacs-lisp
(defun mu4e:guess-folder (base-dir &rest possible-names)
  "Guess that a folder is present based on."
  (or (-first (lambda (it) (-any-p (lambda (name) (s-contains-p name it t)) possible-names))
              (cddr (directory-files base-dir)))
      (-first-item possible-names)))
#+end_src
...
#+begin_src emacs-lisp
(defun mu4e:init-account-context (email)
  "Initialize the context of EMAIL account."
...
#+begin_src emacs-lisp
(defun mu4e:initialize-all-contexts ()
  "Initiaize the context for each account."
  (-each (-map #'pass:email-account-plist (pass:email-list))
    #'mu4e:init-account-context))
#+end_src
