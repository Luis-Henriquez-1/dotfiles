...
(defun package:install (package)
  ...
  (condition-case nil
      (if (package-installed-p (car package))
          (void:log "%s is already installed." (car package))
        (void:log "Installing %s package." (car package))
        (shut-up
          (package-refresh-contents)
          (if (= 1 (length package))
              (package-install (car package))
            (setq package (cl-subst :fetcher :host (car package)))
            (quelpa package))))
    (error (void:log "Could not install %s" package))))
#+end_src
