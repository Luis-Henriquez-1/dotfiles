...
(defhook! exwm:start-in-char-mode-h ()
  ...
  (when (--any-p (string-prefix-p it exwm-instance-name)
                 (list "emacs"))
...
WORD is a string with no whitespace characters."
  (-let* ((vowel-regexp (rx (or "a" "e" "i" "o" "u" "y")))
          (letters (-> word
                       ...
                       (s-split "" it t)))
          (no-vowels (-remove (-partial #'string-match-p vowel-regexp) letters)))
    (->> no-vowels
         (-uniq)
         (-map #'downcase)
         (apply #'concat))))
...
(defun pass:email-account-plist (email)
  ...
  (shut-up!
    (->> (cdr (password-store-parse-entry it))
         (mapcar #'car)
