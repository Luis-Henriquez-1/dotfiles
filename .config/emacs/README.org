...
**** add multiple hooks
:PROPERTIES:
:ID:       bf95b493-7809-42c3-9d81-888bb44ddfaa
:END:

#+begin_src emacs-lisp
(defun void:add-hook (targets hooks &optional depth local)
  "Add HOOKS to TARGETS.
HOOKS is either a hook symbol or a list of hook symbols."
  (dolist (target (-list targets))
    (dolist (hook (-list hooks))
      (add-hook target hook depth local))))
#+end_src

**** add multiple advices
:PROPERTIES:
:ID:       b9816912-7b5d-4477-bb94-b06581e0f4f0
:END:

#+begin_src emacs-lisp
(defun void:add-advice (targets where advices &optional props)
  "Add ADVICES to TARGETS."
  (dolist (target (-list targets))
    (dolist (advice (-list advices))
      (advice-add target where advice props))))
#+end_src
...
#+begin_src emacs-lisp
(define-leaf-keyword! load-incrementally (before :before-call)
...
(defun org/dwim-edit-description ()
  ...
  (-let* ((elements (org:element-headline-elements))
    ...
    (void:add-hook 'edit-indirect-after-creation-hook
