...
#+begin_src emacs-lisp
(defvar void-incremental-package-alist nil
...
(defhook! void-run-start-emacs-hook-h ()
  ...
  (dolist (hook '(void-start-emacs-hook))
    (void-log "Executing %s" hook)
    (unless hook (void-log "%s is nil!" hook))
    (run-hook-wrapped hook #'void-try-run-hook)))
#+end_src
...
****** tangle asynchronously upon saving                              :disabled:
:PROPERTIES:
...
had time to add, or didn't foresee.
...
#+begin_src emacs-lisp
(push '((memq leaf--key (list :setq :pre-setq :setq-default
                              :custom :custom*
...
**** leaf-handler-package-install-fn!

**** leaf-handler-advice-call!

#+begin_src emacs-lisp
(defmacro leaf-handler-advice-call! (package where fns)
  "Generate the code for advising."
  (let ((fname (intern (concat "leaf-load-" (symbol-name package) "-a"))))
    `(progn
       (defun ,fname (&rest _)
         (void-log "Loading %S..." ',package)
         (unless (require ',package nil t)
           (message "Error loading %S" ',package))
         ,@(mapcar (lambda (fn) `(advice-remove #',fn #',fname)) fns)
         (fmakunbound #',fname))
       ,@(mapcar (lambda (fn) `(advice-add #',fn ,where #',fname)) fns))))
#+end_src

**** =before-call= and =after-call= keyword

#+begin_src emacs-lisp
(let ((before '(:before-call `((leaf-handler-advice-call! ,leaf--name :before ,leaf--value)
                               ,@leaf--body)))
      (after '(:after-call `((leaf-handler-advice-call! ,leaf--name :after ,leaf--value)
                             ,@leaf--body))))
  (setq leaf-keywords (leaf-insert-list-before leaf-keywords :hook before))
  (setq leaf-keywords (leaf-insert-list-before leaf-keywords :hook after)))

(push :after-call leaf-defer-keywords)
(push :before-call leaf-defer-keywords)

(push '((member leaf--key '(:after-call :before-call))
...
#+begin_src emacs-lisp
(let ((form '(:void-straight
              `(,@(mapcar (lambda (elm) `(straight:install-package ',elm)) leaf--value)
                ,@leaf--body))))
  (setq leaf-keywords (leaf-insert-list-before leaf-keywords :straight form)))

(push '(:ensure . :void-straight) leaf-alias-keyword-alist)
...
**** :defer-incrementally                                             :disabled:

#+begin_src emacs-lisp
(push :defer-incrementally leaf-defer-keywords)

`(append! void-incremental-packages ,@leaf--value)
(leaf-insert-list-before
    '(:defer-incrementally (push (,leaf-package . ,@leaf-value) void-incremental-package-alist)))
#+end_src

**** os

#+begin_src emacs-lisp
(let ((it '(:os (when leaf--body `((with-os! ,leaf--value ,@leaf--body))))))
  (setq leaf-keywords (leaf-insert-list-before leaf-keywords :when it)))
#+end_src
...
#+begin_src emacs-lisp
(leaf scrot (:host github :repo "dakra/scrot.el"))
#+end_src
...
#+begin_src emacs-lisp
(leaf aggressive-indent :hook emacs-lisp-mode-hook)
#+end_src
...
#+begin_src emacs-lisp
(leaf evil-surround :hook prog-mode-hook text-mode-hook)
#+end_src
...
#+begin_src emacs-lisp
(leaf all-the-icons-dired :hook ranger-mode-hook)
#+end_src
...
#+begin_src emacs-lisp
(leaf git-gutter :commands git-gutter-mode)
#+end_src
...
(leaf autorevert
  ...
  (defhook! void-auto:revert-visible-buffers-h ()
...
#+begin_src emacs-lisp
(leaf sudo-edit :commands sudo-edit)
#+end_src
...
(leaf org-agenda
  ...
  :setq
  (org-agenda-files . (--map (concat org-directory it) '("todo.org" "emacs.org")))
...
#+begin_src emacs-lisp
(leaf counsel-org-capture-string
  :after org ivy
