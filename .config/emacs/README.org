...
(defun void::pcase-pattern (form)
  "Return a pcase-friendly pattern."
...
(defmacro let!! (bindings &rest body)
  ...
  `(pcase-let*
       ,(mapcar (lambda (binding)
                   (list (list '\` (void::pcase-pattern (car binding)))
                         (cadr binding)))
                 bindings)
     ,@body))
#+end_src
...
(defmacro if-let! (bindings then &rest else)
  (declare (indent 2))
  ;; determine whether single binding.
  ;; (when (or (symbolp (car bindings))
  ;;           (length 2 bindings))
...
(let ((font-dir
  ...
  (unless (-all-p (lambda (it) (file-exists-p (concat font-dir it)))
                       '("all-the-icons.ttf"
                         "file-icons.ttf"
                         "fontawesome.ttf"
