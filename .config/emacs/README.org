...
*** mode-symbol-p
:PROPERTIES:
:ID:       e6f7314a-b38a-402e-a406-79fcdcd8b1f6
:END:
...
*** hook-symbol-p
:PROPERTIES:
:ID:       e7c5aeff-0af2-467b-bff9-7d8f219f9e37
:END:
...
*** keymap-symbol-p
:PROPERTIES:
:ID:       cedbd55b-6164-4626-98b1-659e4001a1d6
:END:

#+begin_src emacs-lisp
(defun void-keymap-symbol-p (symbol)
  "Return non-nil if SYMBOL is a keymap symbol."
  (string-match-p (rx "") symbol)
  )
#+end_src
...
(defmacro after! (features &rest body)
  ...
  ;; (setq features (if (listp features)))
...
(defmacro with-symbols! (names &rest body)
  ...
  (declare (indent defun))
  `(let ,(mapcar (fn! `(,<symbol> (make-symbol ,(symbol-name <symbol>)))) names)
     ,@body))
#+end_src
...
Example:
  ...
  (let* ((names-and-forms (seq-map (fn! (if (consp <bind>)
                                            (cons (car <bind>) (cadr <bind>))
                                          (cons <bind> <bind>)))
                                   bindings))
         (names (seq-map #'car names-and-forms))
         (forms (seq-map #'cdr names-and-forms))
         (symbols (seq-map (fn! (make-symbol (symbol-name <name>))) names)))
...
*** let!
:PROPERTIES:
:ID:       3bac36e8-40e8-4838-8aeb-51c9cd85a572
:END:
...
(defmacro fn! (&rest body)
  "Return a lambda."
  ;; check if first argument is a list of anaphoric symbols.
  ;; (and (listp (car ))
  ;;      (seq-every-p #'void:anaphoric-symbol-p body))
  ;; check if anaphoric symbols are numbered.
  ;; check if numbered, otherwise no order.
...
BODY contains any number of `<NAME>' where NAME refers to the name of the symbol."
  ...
  (let* ((symbols (thread-last (flatten-list body)
    ...
    `(let* ((,structure-sym ,structure)
            ,@(-map (-lambda ((sym name)) `(,sym (funcall ,fn ,structure-sym ',name)))
                    (-zip-with #'list symbols names)))
       ,@body)))
#+end_src
...
(defvar void-localleader-alt-key "M-SPC m"
  "The localleader prefix key for major-mode specific commands.")
...
**** TODO ignore advices                                              :disabled:
:PROPERTIES:
:ID:       03438e0c-2780-477a-b78f-5ca93c9e4562
:END:

#+begin_src emacs-lisp
;; list advices
(defun void:funcall-ignore-advices (fn)
  (let ((advices (advice--p (advice--symbol-function 'fn))))
    (general-advice-add))
  )
#+end_src
...
(defun selectrum/buffer-or-recentf ()
  ...
  (let* ((selectrum-should-sort-p nil)
         ...
         (recentf-buffers (-map (lambda (it) (cons (propertize (shrink-path-file it) 'face 'font-lock-comment-face) it))
                                   recentf-list)))
...
(defun void/browser-links ()
  ...
  (let ((links nil)
    ...
    (with-current-buffer (find-file-noselect qutebrowser-history-path)
      ...
      (setq links (-map (lambda (it) (cons (s-truncate 110 it) it)) links))
...
WORDS is a string of non whitespace and whitespace."
  (thread-last (s-split "\s" words t)
    (-map (lambda (it) (char-to-string (aref it 0))))
    (-map #'downcase)
    (apply #'concat)))
#+end_src
...
(defun pass:email-list ()
  ...
  (->> (password-store-list)
       (-map (fn! (elt (s-match "email/\\(.*\\)" <entry>) 1)))
       (-non-nil)))
#+end_src
...
(defun pass:email-account-plist (email)
  ...
  (shut-up!
    (-mapcat (fn! (list (intern <field>)
             ...
             (-map #'car (cdr (password-store-parse-entry it))))))
#+end_src
...
valid `eshell:enabled-sections' and the `eshell-prompt-string'."
  (concat eshell:header
          (thread-last eshell:enabled-sections
            (-map (lambda (it) (void:symbol-intern 'eshell-prompt-- it)))
            (-filter #'fboundp)
...
(defhook! void:unpropertize-kill-ring-h ()
  ...
  :append kill-emacs-hook
  (setq kill-ring
        (-map (lambda (it) (when (stringp it) (substring-no-properties it)))
                 (-non-nil kill-ring))))
#+end_src
...
(defun org:todo-keywords ()
  ...
  (-filter (lambda (it) (and (stringp it) (not (string= "|" it))))
...
(use-feature! org-agenda
  ...
  :setq
  (org-agenda-files . (-map (lambda (it) (concat org-directory it))
                               '("todo.org")))
