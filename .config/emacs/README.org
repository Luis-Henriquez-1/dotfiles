...
(leaf:define-keyword! void-straight (before :straight)
...
(after! helm
  (defun helm:org-capture-candidates ()
    ...
    (let* ((alist (thread-first (org-capture-upgrade-templates org-capture-templates)
      ...
      (-remove (-lambda ((elt &as _ . key))
                 (-all-p (lambda (it) (not (string-prefix-p key it)))
                         (-remove key keys)))
               alist)))
...
(defun selectrum/buffer-or-recentf ()
  ...
  (require 'recentf)
  (let* ((selectrum-should-sort-p nil)
         (live-buffers (--remove (string-match-p (rx bol white) it)
                                 (mapcar #'buffer-name (buffer-list))))
         (recentf-buffers (--map (cons (propertize (shrink-path-file it) 'face 'font-lock-comment-face) it)
                                 recentf-list)))
