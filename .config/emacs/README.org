...
(defmacro alet! (value &rest body)
  ...
  (let ((orign-value (make-symbol "orig-value")))
    `(let* ((,orign-value ,value)
            ,@(seq-map (fn! (list <symbol> orign-value)) (void:anaphoric-symbols body)))
       ,@body)))
#+end_src
...
BODY contains any number of `<NAME>' where NAME refers to the name of the symbol."
  ...
  (let* ((symbols (thread-last (flatten-list body)
    ...
    `(let* ((,structure-sym ,structure)
            ,@(seq-map (fn! `(,<sym> (funcall ,fn ,structure-sym ',<name>)))
...
(defmacro after! (features &rest body)
  ...
  (pcase (car-safe features)
    ('or
     (macroexp-progn
      (seq-map (fn! `(after! ,<feature> ,@body))
               (cdr features))))
    ('and
     (void:wrap-form (seq-map (fn! `(after! ,<feature>)) (cdr features))
                     (macroexp-progn body)))
    ...
    ((pred symbolp)
     `(if (featurep ',features)
          ,(macroexp-progn body)
        (eval-after-load ',features '(with-demoted-errors ,@body))))
     ...
     (if void:backup-fonts
         (progn
           ...
           (setq default-frame-alist
                 (seq-remove (fn! (eq (car <it>) 'font)) default-frame-alist))
           (add-to-list 'default-frame-alist (cons 'font void:font))
...
(defun defsnippet:new-snippet (&rest plist)
  ...
  (let* ((keys '(:key :name :contributor :group :expand-env :binding :type :uuid)))
    (with-plist! plist
      (cl-assert (-all-p (-not #'null) (list <:name> <:mode> <:body>)))
      (alet (append
             (list "# -*- mode: snippet -*-\n")
             (seq-map (lambda (key)
                     (format "# %s: %s\n" (void:keyword-name key) (plist-get plist key)))
                   (-filter (-partial #'plist-get plist) keys))
             (list "# --\n")
...
(use-package! yasnippet
  ...
  :defer-config
  (-each yas-snippet-dirs (fn! (mkdir <> t)))
...
(defun selectrum/buffer-or-recentf ()
  ...
  (require 'recentf)
  (let* ((selectrum-should-sort-p nil)
         (live-buffers (-remove (lambda (it) (string-match-p (rx bol white) it))
...
(defun selectrum/org-set-tag ()
  ...
  (let* ((org-use-fast-tag-selection nil)
    ...
    (org-set-tags
     (append (-remove (lambda (it) (member it current-tags)) selected-tags)
             (-remove (lambda (it) (member it selected-tags)) current-tags)))))
#+end_src
...
(defun mu4e/init ()
  ...
  (let ((email-dirs (seq-map (fn! (concat VOID:EMAIL-DIR <email>)) (pass:email-list))))
...
(defun pass:email-list ()
  ...
  (thread-last (password-store-list)
    (seq-map (fn! (elt (s-match "email/\\(.*\\)" <entry>) 1)))
    (-non-nil)))
#+end_src
...
(defun pass:email-account-plist (email)
  ...
  (shut-up!
    (-mapcat (fn! (list (intern <field>)
                        (password-store-get-field (concat "email/" email) <field>)))
             (seq-map #'car (cdr (password-store-parse-entry it))))))
#+end_src
...
If there is no match, return POSSIBLE-NAME."
  (alet (or (-first (lambda (it) (-any-p (lambda (name) (s-contains-p name it t))
...
(defun mu4e::account-context (email)
  ...
  (-let ((base-dir (concat VOID:EMAIL-DIR email "/"))
         (name (-second-item (s-match ".*@\\([^.]*\\)" email)))
         ((&plist 'out-host 'out-port) (pass:email-account-plist email)))
...
(defadvice! mu4e:initialize-all-contexts ()
  ...
  :before mu4e
  (require 'password-store)
  (--each (seq-map #'mu4e::account-context (pass:email-list))
...
(defun org:dwim-description-bounds (point elements)
  ...
  (acond
   ...
   ((-first (lambda (it) (eq 'property-drawer (org-element-type it)))
               (reverse elements))
...
And go to the start of the path selected by the headline."
  ...
  (let ((selectrum-should-sort-p nil)
    ...
    (thread-first (seq-map #'car paths)
