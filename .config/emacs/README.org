...
**** TODO =before-call= keyword

#+begin_src emacs-lisp
(let ((form '(:before-call
              (let ((fname (intern (concat "leaf:load-" (symbol-name leaf--name)))))
               `((defadvice! ,fname (&rest _)
                   :before ,leaf--value
                   (void-log "Loading %S..." ',leaf--name)
                   (unless (require ',leaf--name nil t)
                    (message "Error loading %S" ',leaf--name))
                   ,@(mapcar (lambda (fn) `(advice-remove ',fn #',fname)) leaf--value)
                   (fmakunbound #',fname))
                 ,@leaf--body)))))
  (setq leaf-keywords (leaf-insert-list-before leaf-keywords :hook form)))

(push '((eq leaf--key :before-call)
...
(leaf shackle
  :hook emacs-startup-hook
  :defer-config
  (push (list (rx "*" (opt "e") "shell") :regexp t :align t :size 0.4))
...
#+begin_src emacs-lisp
(leaf smartparens
  :hook (emacs-lisp-mode-hook prog-mode-hook eshell-mode-hook ielm-mode-hook)
  :setq
