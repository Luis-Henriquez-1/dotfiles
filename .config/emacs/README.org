...
(defmacro dolist! (specs &rest body)
  ...
  (let!! (((pattern list) specs)
          (pcase-friendly-pattern (list '\` (void::pcase-pattern pattern))))
    `(pcase-dolist (,pcase-friendly-pattern ,list)
       ,@body)))
#+end_src
...
(defun defsnippet:new-snippet (&rest plist)
  ...
  (let* ((keys '(:key :name :contributor :group :expand-env :binding :type :uuid)))
    (with-plist! plist
      (cl-assert (seq-every-p (fn! (not (null <it>))) (list <:name> <:mode> <:body>)))
      (alet (append
             (list "# -*- mode: snippet -*-\n")
             (mapcar (lambda (key)
                       (format "# %s: %s\n" (void:keyword-name key) (plist-get plist key)))
                     (seq-filter (fn! (plist-get plist <it>)) keys))
             (list "# --\n")
