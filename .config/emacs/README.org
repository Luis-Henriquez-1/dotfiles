...
#+begin_src emacs-lisp
(defun package:install-package (package-or-recipe)
  "Install package."
  (unless (package-installed-p package)
    (package-refresh-contents)
    (package-install package))
  ;; if it's a recipe install with quelpa.
  ;; (quel)
  )
#+end_src
...
#+begin_src emacs-lisp
(let ((package-user-dir (concat VOID:PACKAGES-DIR)))
  (package-initialize)
  (require 'package)
  (package-install 'quelpa)
  (require 'quelpa)
  (dolist (package '(leaf leaf-keywords dash anaphora))
    (quelpa package)
    ;; (require package)
    ))
(leaf-keywords-init)
