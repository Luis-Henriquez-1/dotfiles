...
*** swiper helm
:PROPERTIES:
:ID:       4e0768d8-706a-4692-b700-2640dd534587
:END:

#+begin_src emacs-lisp
(use-package! swiper-helm :commands swiper-helm)

(defadvice! swiper-helm:speedup (orig-fn &rest args)
  :around swiper-helm
  (let ((gc-cons-threshold VOID:GC-CONS-THRESHOLD-MAX)
        (gc-cons-percentage VOID:GC-CONS-PERCENTAGE-MAX))
    (apply orig-fn args)))
    ...
    (let* ((alist (--> (org-capture-upgrade-templates org-capture-templates)
      ...
      (-remove (-lambda ((elt &as _ . key))
                 (-all-p (lambda (it) (not (string-prefix-p key it)))
                         (-remove (-partial #'string= key) keys)))
               alist)))
...
(use-package! ace-jump-helm-line
  :init
  (define-key! '(insert emacs) helm-map "C-'" #'ace-jump-helm-line)
  :setq
...
*** helm-rg
:PROPERTIES:
:ID:       5966387c-cb9e-4d18-8b54-97240864862b
:END:
...
#+begin_src emacs-lisp
(define-key! 'normal mu4e-headers-mode-map
...
(define-key! 'normal mu4e-view-mode-map
  "q" #'kill-buffer-and-window)
...
#+begin_src emacs-lisp
(define-key! 'normal w3m-mode-map
  "o" #'w3m-search
...
Youtube contains a *huge* library of videos. It would be a shame not to take
advantage of it.
...
(use-package! ytdl
  ...
  (ytdl-video-folder . VOID:VIDEO-DIR)
...
*** ytel
:PROPERTIES:
:ID:       69d6a52c-137b-4b7c-aa23-75e95399594d
:END:
...
**** utility functions
:PROPERTIES:
:ID:       21924123-35ce-4cf2-bf6a-2d24708cbc32
:END:

***** TODO search a video with completion framework
:PROPERTIES:
:ID:       6ba6baa7-e0f8-457e-a777-0fba05c6d81d
:END:

#+begin_src emacs-lisp
#+end_src

***** stream a video in mpv
:PROPERTIES:
:ID:       1109a6f4-3c74-4ddb-963a-1434c421b542
:END:

#+begin_src emacs-lisp
(defun ytel/watch-video ()
...
***** TODO download audio
:PROPERTIES:
:ID:       1091dca6-698d-4bda-bcea-54e72092621b
:END:

#+begin_src emacs-lisp
(defun ytel/download-audio ()
  "Stream video at point in mpv."
  (interactive)
  (let ((id (ytel-video-id (ytel-get-current-video)))
        (fname (read-string "Filename: ")))
    (ytdl--download-async (concat "https://www.youtube.com/watch?v=" id)
                          (concat VOID:MUSIC-DIR fname)
                          ytdl-music-extra-args
                          nil
                          "Music"))
  (message "Audio downloading..."))
#+end_src

***** thumbnail
:PROPERTIES:
:ID:       6f8c8765-abcd-4432-8f34-1b12e4b81620
:END:

#+begin_src emacs-lisp
(defun ytel/download-thumbnail ()
  ""
  (interactive)
  )
#+end_src
...
#+begin_src emacs-lisp
(define-key! :states '(emacs insert) [escape] #'evil-force-normal-state)
...
(use-package! key-chord
  ...
  (define-key! :states '(visual insert)
...
#+begin_src emacs-lisp
(define-key! 'normal ranger-mode-map
  "m" #'dired-mark
  "d" #'dired-do-delete
  "s" #'dired-do-symlink
  "e" #'eshell
...
#+begin_src emacs-lisp
(define-key! 'normal pdf-view-mode-map
  "j" #'pdf-view-next-line-or-next-page
  "k" #'pdf-view-previous-line-or-previous-page
...
(use-package! outshine
  :hook emacs-lisp-mode-hook
  :defer-config
  (define-key! '(normal) emacs-lisp-mode-map
...
Buffers can easily grow unwieldy and out of control. For too long my buffer list has had.
Some of my references: [[https://emacs-fu.blogspot.com/2010/03/cleaning-up-buffers-automatically.html][emacs-fu]], [[https://www.reddit.com/r/emacs/comments/5d414w/how_do_i_keep_my_buffers_list_clean/][emacs-reddit]].
...
different times. However, it is not difficult to get around this.

#+begin_src emacs-lisp
(use-feature! midnight
  :commands clean-buffer-list)
#+end_src

*** tempbuff
:PROPERTIES:
:ID:       92623b8e-787b-48d9-9b4f-b928abca0471
:END:

[[https://www.emacswiki.org/emacs/TempbufMode][This package]] provides a minor mode that kills buffers after it has been
determined that it is no longer useful. It is determined whether its not useful
based on whether functions have been called in it and whether it is visible.

#+begin_src emacs-lisp
(use-package! tempbuf
  :hook (find-file-hooks . turn-on-tempbuf-mode))
#+end_src
...
(define-key! 'normal ";" #'execute-extended-command
  "q" #'delete-window)
#+end_src
...
(define-leader-key!
  ...
  "s" (list :def #'swiper-helm :wk "swiper"))
