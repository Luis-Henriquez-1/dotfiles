...
(define-leaf-keyword! ignore-buffers (before :ignore-buffers)
  `((--each ',leaf--value
      (add-to-list 'clean-buffer-list-kill-regexps it))
    ,@leaf--body))
#+end_src
...
determined that it is no longer useful.

#+begin_src emacs-lisp
(use-package! tempbuff
