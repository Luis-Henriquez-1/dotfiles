...
**** unfold
:PROPERTIES:
:ID:       e804a7c5-9cdd-4b52-95a4-bc4e8af30933
:END:

#+begin_src emacs-lisp
(defun seq:unfold (fun seed)
  (let ((last (funcall fun seed))
        (r nil))
    (while last
      (push (car last) r)
...
**** pcase pattern
:PROPERTIES:
:ID:       a26885f3-3dae-40a7-bec0-319b330dab87
:END:

[[][]] is a very flexible macro. Instead of re-inveinting destructuring using
dash. The only annoyance is that the syntax while flexible and powerful is also
unpleasant-looking and cumbersome. I'd like to use the syntax that I want, have
that converted to pcase-friendly syntax, and finally pass that in to pcase
destructuring macros.

#+begin_src emacs-lisp
(defun void:pcase-pattern (form)
  "Return pcase pattern for form."
  (cl-labels ((void::pcase-pattern (form)
                                   (cond ((equal nil form) form)
                                         ((listp form)
                                          (cons (void::pcase-pattern (car form))
                                                (void::pcase-pattern (cdr form))))
                                         ((vectorp form)
                                          (seq-into (cons (void::pcase-pattern (seq-first form))
                                                          (void::pcase-pattern (seq-into (seq-rest form) 'list)))
                                                    'vector))
                                         ((and (symbolp form) (not (equal '_ form)))
                                          (list '\, form))
                                         (t form))))
    (list '\` (void::pcase-pattern form))))
#+end_src
...
**** cons-pair-p
:PROPERTIES:
:ID:       3320ad2b-3afe-429b-a2f8-57fb7ed711a3
:END:

In emacs-lisp =listp= returns non-nil for cons-cells. I'd like a distinction.

#+begin_src emacs-lisp
(defun void:cons-pair-p (obj)
  "Return non-nil if OBJ is true objs pair.
...
**** void real listp
:PROPERTIES:
:ID:       5cb6286d-2d4b-48d4-842a-9f9d0cc35eee
:END:

I'd also like to know when a list is actually a list I can loop through.

#+begin_src emacs-lisp
(defun void:real-list-p (obj)
  "Return non-nil if obj is a list."
  (declare (pure t) (side-effect-free t))
  (and (listp obj) (not (void:cons-pair-p obj))))
#+end_src

**** enlist
:PROPERTIES:
:ID:       61095d7b-749e-4832-98b7-c2aaadbca7bb
:END:

#+begin_src emacs-lisp
(defun void:enlist (obj)
  "Make object into a list."
  (declare (pure t) (side-effect-free t))
  (if (void:real-list-p obj) obj (list obj)))
#+end_src
...
(defun void:keyword-name (key)
  ...
  (declare (pure t) (side-effect-free t))
  (substring-no-properties (void:to-string key) 1))
#+end_src
...
(defmacro with-symbols! (names &rest body)
  ...
  `(let ,(mapcar (lambda (symbol) `(,symbol (make-symbol ,(symbol-name symbol)))) names)
     ,@body))
#+end_src
...
Bare symbols in BINDINGS are equivalent to:
  ...
  (let* ((bind-fn (lambda (bind)
                    (if (consp bind)
                      ...
                      (cons bind bind))))
         (names-and-forms (seq-map bind-fn bindings))
         (names (seq-map #'car names-and-forms))
         (forms (seq-map #'cdr names-and-forms))
         (symbols (seq-map (lambda (name) (make-symbol (symbol-name name))) names)))
    `(with-symbols! ,symbols
       (list 'let
             (seq-mapn #'list (list ,@symbols) (list ,@forms))
             ,(cl-list* 'let
                        (seq-mapn #'list names symbols)
                        body)))))
#+end_src

**** destructuring
:PROPERTIES:
:ID:       ef1c0e0e-9a2f-4ae3-89ff-f404772ee1b8
:END:
...
***** let!
:PROPERTIES:
:ID:       c53ec463-30ba-4ca6-95a2-79f6fee68539
:END:

Macro to replace Dash's =-let=.

#+begin_src emacs-lisp
(defmacro let! (bindings &rest body)
  "Wrapper around `pcase-let` with friendlier pattern matching."
  (declare (indent defun))
  (let ((var-to-new (mapcar (lambda (var) (cons var (make-symbol (symbol-name var))))
                            (seq-uniq (flatten-list (mapcar #'car bindings))))))
    `(let!! ,(append
              (seq-map (lambda (it) (cons (cl-sublis var-to-new (car it))
                                          (cdr it)))
                       bindings)
              (seq-map (lambda (it) (list (car it) (cdr it)))
                       var-to-new))
       ,@body)))
#+end_src
...
***** let!!
:PROPERTIES:
:ID:       742e5a6c-8e5b-4ea0-a3d1-9dfd352d0b3e
:END:
...
(defmacro let!! (bindings &rest body)
  ...
  `(pcase-let*
       ,(mapcar (lambda (binding)
                  (list (void:pcase-pattern (car binding))
                        (cadr binding)))
                bindings)
     ,@body))
#+end_src
...
***** if-let!!
:PROPERTIES:
:ID:       4f1c72b6-acfb-44b7-ab5f-c24bc6dd17dd
:END:

#+begin_src emacs-lisp
(defmacro if-let! (bindings then &rest else)
  (declare (indent 2))
  (when (= 2 (length bindings))
    (setq bindings (list bindings)))
  `(let! ,bindings
     (if (and ,@(flatten-list (seq-map #'car bindings)))
         ,then
       ,@else)))
...
***** when-let!!
:PROPERTIES:
:ID:       90084108-8bbe-492a-80fb-da37accaea29
:END:
...
***** dolist!
:PROPERTIES:
:ID:       4f92497f-bc72-49d0-874d-c0876dbc86f8
:END:

#+begin_src emacs-lisp
(defmacro dolist! (specs &rest body)
  "Same as `pcase-dolist' but with nicer syntax."
  (declare (indent defun))
  (let!! (((pattern list) specs))
    `(cl-block nil
       (pcase-dolist (,(void:pcase-pattern pattern) ,list)
         ,@body))))
#+end_src

***** defun!                                                          :disabled:
:PROPERTIES:
:ID:       9bb1621a-2315-4c08-a2b4-0f62e15202d2
:END:
...
***** lambda!
:PROPERTIES:
:ID:       9acda4bb-e1c4-42d1-a6f6-9cdad5d1b15e
:END:

#+begin_src emacs-lisp
(defmacro lambda! (spec &rest body)
  "Only accepts one argument. Made to be used with mapcar."
  (declare (indent defun))
  (with-symbols! (args)
    (when (= (length spec) 1)
      (setq spec (car spec)))
...
***** fn!
:PROPERTIES:
:ID:       1cf5fc1c-cabc-4e32-8e74-2be2a3cd48a6
:END:
...
(defmacro fn! (&rest body)
  ...
  (with-symbols! (args)
    `(lambda (,args)
       (let! ((,(void:anaphoric-symbols body) (void:enlist ,args)))
         ,@body))))
#+end_src

***** alet!
:PROPERTIES:
:ID:       64b57fb6-11b5-46c7-b002-1497aa4b2edb
:END:

#+begin_src emacs-lisp
(defmacro alet! (value &rest body)
  (declare (indent defun))
  (let ((orign-value (make-symbol "orig-value")))
    `(let* ((,orign-value ,value)
            ,@(mapcar (fn! (list <symbol> orign-value)) (void:anaphoric-symbols body)))
       ,@body)))
#+end_src

***** aif!
:PROPERTIES:
:ID:       7d84b516-76f1-48e4-9897-04a2795c2d1d
:END:
...
***** awhen!
:PROPERTIES:
:ID:       3953606e-7301-418e-ad05-fd909b28a050
:END:
...
BODY contains any number of `<NAME>' where NAME refers to the name of the symbol."
  ...
  (let* ((symbols (thread-last (flatten-list body)
                    (seq-filter #'void:anaphoric-symbol-p)
                    (seq-uniq)))
         (names (mapcar #'void:anaphoric-true-symbol symbols))
         (structure-sym (make-symbol "structure")))
    `(let* ((,structure-sym ,structure)
            ,@(mapcar (fn! `(,<sym> (funcall ,fn ,structure-sym ',<name>)))
                      (cl-mapcar #'list symbols names)))
       ,@body)))
#+end_src
...
(defmacro defadvice! (name args &rest body)
  ...
  (let! ((docstring (when (stringp (car body)) (pop body)))
    ...
    `(prog1 (defun ,name ,args ,docstring ,@body)
       ,@(mapcar (fn! `(advice-add #',<advice> ,key #',name))
                 (void:enlist advices)))))
#+end_src
...
(defmacro defhook! (name args &rest body)
  ...
  (let!! ((docstring (when (stringp (car body)) (pop body)))
          ...
          (local (eq key :local)))
    `(prog1 (defun ,name ,args ,docstring ,@body)
       ,@(mapcar (fn! `(add-hook ',<hook> #',name ,append ,local))
                 (void:enlist hooks)))))
#+end_src
...
**** if!
:PROPERTIES:
:ID:       e2e9b33d-9a07-44d7-85ec-71bfd6a4c678
:END:
...
**** when!
:PROPERTIES:
:ID:       f8e9d68b-264a-44f3-8847-3e41689384aa
:END:

#+begin_src emacs-lisp
(defmacro when! (cond &rest body)
  "Expand to BODY if CONDITION is non-nil at compile/expansion time.
...
**** quiet!                                                           :disabled:
:PROPERTIES:
:ID:       57798d18-2c85-45a5-b3f8-ca0397d4b99e
:END:

#+begin_src emacs-lisp
(defmacro quiet! (&rest body)
  "Run BODY without generating output."
  (let! ((standard-output (lambda (&rest _)))
...
(defmacro after! (features &rest body)
  ...
  (pcase (car-safe features)
    ('or
     (macroexp-progn
      (mapcar (fn! `(after! ,<feature> ,@body))
              (cdr features))))
    ('and
     (void:wrap-form (mapcar (fn! `(after! ,<feature>)) (cdr features))
                     (macroexp-progn body)))
...
**** while!
:PROPERTIES:
:ID:       e4fc8ee2-90ff-49c7-b586-4fc2da6ea1bf
:END:

=cl-loop= can be completely replaced by =mapcar=. The only time where I ever
miss =cl-loop= is when I'd like to "collect" values.

#+begin_src emacs-lisp
(defmacro while! (test &rest body)
  "Same as while but return collected values in a list."
  (declare (indent defun))
  `(cl-block nil
     (while ,test ,@body)))
#+end_src
...
*** keybinding                                                        :disabled:
:PROPERTIES:
:ID:       e8f85c93-2699-4ec9-baf4-ac2b109ca030
:END:

**** leaders
:PROPERTIES:
:ID:       c0bffef4-1d78-4cdf-825d-fc628c79ea77
:END:

***** leaders
:PROPERTIES:
:ID:       9e4eceb6-bdc8-496e-abe5-aaf2f5ab9d2c
:END:
...
***** leader map
:PROPERTIES:
:ID:       2aec7943-ecca-4db4-bc27-1ac565681419
:END:

#+begin_src emacs-lisp
(defvar void-leader-map (make-sparse-keymap)
  "An overriding keymap for <leader> keys.")

(define-prefix-command 'void/leader 'void-leader-map)

(after! evil
  (define-key void-leader-map [override-state] 'normal)
  (define-key void-leader-map [override-state] 'visual)
  (define-key void-leader-map [override-state] 'motion)
  (evil-define-key* '(normal visual motion) global-map (kbd void-leader-key)
                    'void/leader))
#+end_src

***** leader alt map
:PROPERTIES:
:ID:       da4c7933-e479-42e6-8f95-d65a4bcddfe5
:END:

#+begin_src emacs-lisp
(defvar void-leader-alt-map (make-sparse-keymap)
  "An overriding keymap for <leader> keys.")

(define-prefix-command 'void/leader-alt 'void-leader-alt-map)

(after! evil
  (define-key void-leader-alt-map [override-state] 'insert)
  (define-key void-leader-alt-map [override-state] 'emacs)
  (evil-define-key* '(insert emacs) global-map (kbd void-leader-alt-key)
                    'void/leader-alt))
#+end_src

***** localleader
:PROPERTIES:
:ID:       1343a6ed-6590-47b3-b423-bb49338fbaaa
:END:
...
(defconst VOID-LOCALLEADER-ALT-KEY "M-SPC m"
  "The localleader prefix key for major-mode specific commands.")

(defconst VOID-LOCALLEADER-SHORT-KEY ","
  "An shorter alternative `void-localleader-key'.")

(defconst VOID-LOCALLEADER-SHORT-ALT-KEY "M-,"
  "An shorter alternative for `void-localleader-alt-key'.")
#+end_src

**** evil states
:PROPERTIES:
:ID:       4276f7d5-05a4-4895-9b2e-52d2fa37b54f
:END:

***** list of evil states
:PROPERTIES:
:ID:       df03cf22-5fc8-4145-8aca-0f2708190257
:END:

#+begin_src emacs-lisp
(defvar void-evil-state-alist
  '((n . normal)
    (v . visual)
    (i . insert)
    (e . emacs)
...
***** evil keyword to states
:PROPERTIES:
:ID:       2f3546d4-2f15-46ef-bbf2-ec5ba4802115
:END:

#+begin_src emacs-lisp
(defun void-evil-keyword-to-states (evil-keyword)
  "Converts EVIL-KEYWORD into a list of evil states."
  (--> evil-keyword
       (symbol-name it)
       (substring-no-properties it 1)
       (split-string it "" t)
       (--sort (< (aref it 0) (aref other 0)) it)
       (mapcar #'intern it)
       (mapcar (-rpartial #'-al-get void-evil-state-alist) it)))
#+end_src

***** evil keyword-p
:PROPERTIES:
:ID:       70cc9979-cb88-4960-9509-c1fc3b2256ec
:END:

#+begin_src emacs-lisp
(defun void-evil-keyword-p (obj)
  "Whether obj is an evil keyword."
  (member))
#+end_src

**** define-key macro
:PROPERTIES:
:ID:       df3c6d92-9b6c-40c0-b9ce-aa8d71e40224
:END:

***** list of binding data
:PROPERTIES:
:ID:       6442b9d1-4b74-4925-a5c6-925a191a81f4
:END:

The basis of the binding is a list ~(states map key def)~.

#+begin_src emacs-lisp
(defun kbd::list-data (states maps bindings)
  "Return a list whose elements are (states map key bind)."
  (mapcar (lambda! ((map (key def)))
            (list states map key def))
          (-table-flat #'list maps (seq-partition bindings 2))))
#+end_src

***** apply prefix
:PROPERTIES:
:ID:       75bac321-56e9-45c0-ac22-a27f39664462
:END:

#+begin_src emacs-lisp
(defun kbd::apply-prefix (datum)
  "Return binding with prefix applied."
  (vectorp ())
  (cl-case)
  (setf (cadr binding) 3))
#+end_src
...
***** apply kbd
:PROPERTIES:
:ID:       a2ba300a-458c-433a-9e16-f0b1509d69a8
:END:
...
***** produce binding
:PROPERTIES:
:ID:       04f66c21-1c53-4884-90f7-599cbae0989c
:END:

#+begin_src emacs-lisp
(defun kbd::data-to-binding (datum)
  (pcase-let ((`(,states ,map ,key ,def) datum))
    (cond (states
           `(evil-define-key ',states ,map ,key ,def))
          (t
           `(define-key ,map ,key ,def)))))
#+end_src

***** generate lazy loaded bindings
:PROPERTIES:
:ID:       d8fe6c10-7920-442a-b983-20bfb8ea9da9
:END:

#+begin_src emacs-lisp
(defun kbd::gen-lazy-loaded ()
  "Return lazy loaded binding forms."
  (mapcar (lambda! ((keymap bindings))
            `(after! mode ,@(-map #'produce bindings)))
          (seq-group-by #'cl-second bindings)))
#+end_src

***** generate autoload
:PROPERTIES:
:ID:       4c6eb7e1-d842-46bf-b480-d5622dfd9226
:END:

#+begin_src emacs-lisp
(defun kbd::gen-autoload (binding &optional package interactive type)
  "Returns autoload from package."
  `(autoload function "filename" "" t 'function))
#+end_src

***** process arguments
:PROPERTIES:
:ID:       c48bc608-ceca-465d-bef1-aede14fe216e
:END:

#+begin_src emacs-lisp
(defun kbd::process-arguments (args)
  "Convert list of arguments to `define-key!' into a plist."
  (list
   ;; see if states is passed in.
   :states (or (void-evil-state-p (car args))
               (seq-contains-p ))
   ;; see if map is passed in.
   :maps (or (void-map-symbol-p)
             (seq-contains-p ))
   :bindings
   ())
  )
...
***** macro
:PROPERTIES:
:ID:       32fba3bf-8f3b-4b9e-997d-15b92ff28c5a
:END:

While I also have a function that can bind keys. The implementation was made
with macroexpansion in mind.

#+begin_src emacs-lisp
(defmacro define-key! (&rest args)
  (let* ((maps (if maps maps '(global-map)))
         (data (kbd::list-data states maps bindings)))
    (with-plist! (kbd::process-arguments args)
      `(progn
         ,@(-map #'kbd::data-to-binding
                 (kbd::list-data <states> <maps> <bindings>))))))
#+end_src
...
**** go to project page                                               :disabled:
:PROPERTIES:
:ID:       90f52a97-4bbf-44c2-bac2-7b5f3582650b
:END:

#+begin_src emacs-lisp
(defun straight:cd ()
  (straight-recipes-retrieve (intern (car (straight-recipes-list)))))
#+end_src
...
is the form that will be added to."
  ...
  ;; update aliases
  (dolist (alias (void:enlist alias))
    (add-to-list 'leaf-alias-keyword-alist (cons alias name)))
...
#+begin_src emacs-lisp
(leaf:define-keyword
 :name 'void-leaf-defer
 :where '(before :leaf-defer)
 :alias (list :leaf-defer)
 :form
...
**** make leaf defer default
:PROPERTIES:
:ID:       bdaa5011-f0b3-44bc-a3c9-f9713d82c280
:END:

#+begin_src emacs-lisp
(ignore! (add-to-list 'leaf-defer-keywords :leaf-defer)
         (append! leaf-defaults '(:leaf-defer t)))
#+end_src
...
**** anaphora
:PROPERTIES:
:ID:       c478b00e-bd13-4640-9a9d-f8dd97510d2a
:END:
...
#+begin_src emacs-lisp
 (defhook! exwm:start-in-char-mode-h ()
  ...
  (when (seq-some-p (fn! (string-prefix-p <prefix> exwm-instance-name))
                    (list "emacs"))
...
(defun defsnippet:new-snippet (&rest plist)
  ...
  (let* ((keys '(:key :name :contributor :group :expand-env :binding :type :uuid)))
    (with-plist! plist
      (cl-assert (seq-every-p (fn! (not (null <it>)))
                              (list <:name> <:mode> <:body>)))
      (alet! (append
              (list "# -*- mode: snippet -*-\n")
              (mapcar (lambda (key)
                        (format "# %s: %s\n" (void:keyword-name key) (plist-get plist key)))
                      (seq-filter (lambda (it) (plist-get plist it)) keys))
              (list "# --\n")
...
(defun defsnippet:compute-body (body placeholders)
  ...
  (alet! (mapcar (lambda (it)
                    (let! (((number name form) it))
                      (if (eq '_  name)
...
(use-package! yasnippet
  ...
  :defer-config
  (seq-each (fn! (mkdir <dir> t)) yas-snippet-dirs)
...
(use-package! yasnippet-snippets
  :after yasnippet
  :init
  (awhen! (seq-find (fn! (s-contains-p "yasnippet-snippets" <path>)) load-path)
    (push <path> yas-snippet-dirs)))
#+end_src
...
(after! helm
  (defun helm:org-capture-candidates ()
    ...
    (let* ((alist (thread-first (org-capture-upgrade-templates org-capture-templates)
                    ...
                    (mapcar (-lambda ((name key)) (cons key name)) it)))
           (keys (mapcar #'cdr alist)))
      (cl-remove-if (-lambda ((elt &as _ . key))
...
(defun selectrum/buffer-or-recentf ()
  ...
  (let* ((selectrum-should-sort-p nil)
         ...
         (recentf-buffers (mapcar (lambda (it) (cons (propertize (shrink-path-file it) 'face 'font-lock-comment-face) it))
                                   recentf-list)))
...
(defun void/browser-links ()
  ...
  (let ((links nil)
    ...
    (with-current-buffer (find-file-noselect qutebrowser-history-path)
      ...
      (setq links (mapcar (lambda (it) (cons (s-truncate 110 it) it)) links))
...
WORDS is a string of non whitespace and whitespace."
  (thread-last (s-split "\s" words t)
    (mapcar (lambda (it) (char-to-string (aref it 0))))
    (mapcar #'downcase)
    (apply #'concat)))
#+end_src
...
WORD is a string with no whitespace characters."
  (let!* ((vowel-regexp (rx (or "a" "e" "i" "o" "u" "y")))
          (letters (thread-first word
                     (s-chop-suffixes abbrev:suffixes it)
                     (s-split "" it t)))
          (no-vowels (seq-remove (fn! (string-match-p vowel-regexp <string>))
                                 letters)))
...
(defun abbrev/delete-abbrev ()
  ...
  (let!! ((selected (completing-read
                     "abbrev to delete:"
                     (mapcar (-lambda ((abbrev . expansion))
...
(defun mu4e/init ()
  ...
  (let ((email-dirs (mapcar (fn! (concat VOID:EMAIL-DIR <email>)) (pass:email-list))))
    (when (or (not (seq-every-p #'f-exists-p email-dirs))
              (seq-some-p #'f-empty-p email-dirs))
      (message "creating directories that don't exist.")
      (dolist (dir email-dirs) (mkdir dir t))
...
(defun pass:email-list ()
  ...
  (thread-last (password-store-list)
    (mapcar (fn! (elt (s-match "email/\\(.*\\)" <entry>) 1)))
    (seq-every-p (fn! (not (null <it>))))))
#+end_src
...
(defun pass:email-account-plist (email)
  ...
  (shut-up!
    (mapcan (fn! (list (intern <field>)
                       (password-store-get-field (concat "email/" email) <field>)))
            (mapcar #'car (cdr (password-store-parse-entry it))))))
#+end_src
...
If there is no match, return POSSIBLE-NAME."
  (alet! (or (seq-find (lambda (it) (seq-some-p (fn! (s-contains-p <name> it t))
                                                (cons possible-name other-possible-names)))
                       (cddr (directory-files base-dir)))
             possible-name)
    (format "/%s/%s" (f-filename base-dir) <folder>)))
#+end_src
...
#+begin_src emacs-lisp
(defadvice! mu4e:initialize-all-contexts ()
  "Initiaize context for each email account."
  :before mu4e
  (require 'password-store)
  (seq-each (fn! (cl-pushnew <context> mu4e-contexts))
            (mapcar #'mu4e::account-context (pass:email-list))))
#+end_src
    ...
    (when (org-at-heading-p)
      (cons (org-element-at-point)
            (ignore-errors
              (org-down-element)
              (seq-unfold (lambda (x)
                            (when (ignore-errors (org-forward-element))
                              (cons x (org-element-at-point))))
...
ELEMENTS is a list of headline elements (see `org:element-headline-elements')."
  (awhen! (seq-find (fn! (seq-some-p (apply-partially #'org:element-contains-point-p point)
                                     <it>))
                    (thread-last (cdr elements)
                      (seq-group-by #'org:element-paragraph-p)
                      (seq-filter (lambda (it) (org:element-paragraph-p (car it))))))
    (cons (org-element-property :contents-begin (cl-first <it>))
          (org-element-property :contents-end (car (last <it>))))))
#+end_src
...
(defun org:dwim-description-bounds (point elements)
  ...
  (require 'dash)
  ;; case 1
  ;; (seq-find (fn! (and (org:element-paragraph-p <elt>)
  ;;                     (org:element-contains-point-p (point) <elt>)))
  ;;           elements)
  ...
  (acond
   ...
   ((-some->> elements
      ...
      (seq-find #'org:element-paragraph-p)
      (org-element-property :begin))
    (org:edit-desc-bounds it elements))
   ;; Skip property drawers.
   ((seq-find (fn! (eq 'property-drawer (org-element-type <element>)))
              (reverse elements))
    (alet (org-element-property :end it) (cons it it)))
   ;; There's no paragraphs or property drawers.
   ((save-excursion (goto-char (org-element-property :begin (cl-first elements)))
                    (end-of-line)
                    (org-skip-whitespace)
                    (point))
    (cons it it))))
...
(defun org/dwim-edit-source-block ()
  ...
  (if (or (org-in-src-block-p)
          (thread-last (org:element-headline-elements)
            (seq-find (lambda (it) (eq (org-element-type it) 'src-block)))
            (org-element-property :begin)
            (goto-char)))
...
#+begin_src emacs-lisp
(after! em-term
  (seq-each (fn! (add-to-list 'eshell-visual-commands <command>))
            '("tmux" "htop" "bash" "zsh" "fish" "vim" "nvim" "ncmpcpp")))
#+end_src
...
(use-feature! recentf
  ...
  (recentf-exclude . (list #'file-remote-p
                           ...
                           #'(lambda (file)
                               (seq-some-p (apply-partially #'file-in-directory-p file)
                                (list VOID:DATA-DIR)))))
...
(use-feature! autorevert
  ...
  (defhook! auto-revert:visible-buffers-h ()
    ...
    (mapc (lambda (it) (with-current-buffer it (auto-revert:buffer-h)))
...
(defadvice! pdf-tools:build-pdf-info-program-a ()
  ...
  (unless (file-executable-p pdf-info-epdfinfo-program)
    (let ((wconf (current-window-configuration)))
      ...
      (seq-each (fn! (with-current-buffer it
