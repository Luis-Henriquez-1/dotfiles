...
Here I initialize the package managers I could use, choose one based on either
preference or the availability of git, and then install only the essential
external packages.

*** package.el
:PROPERTIES:
:ID:       89b26461-2338-4842-8b03-218dc095af32
:END:

=package.el= is Emacs's built-in package manager.

**** set up
:PROPERTIES:
:ID:       05f0eb53-d2b5-4288-a9d5-9aa35be49a1b
:END:
...
**** install a package
:PROPERTIES:
:ID:       58ad377f-ecd0-44de-9451-4674bbbcbc05
:END:

For package management I use a combination of =package.el= and =quelpa=.

#+begin_src emacs-lisp
(defun package:install (package)
  "Install PACKAGE.
PACKAGE is either a package name (represented a symbol) or a recipe (represented
as a list). Silence any messages outputed from installing package. If
`void:debug-p' is non-nil, output messages about whether the package was
installed or not."
  (setq package (if (atom package) (list package) package))
  (let ((package (or (car-safe package) package))
        (recipe (when (listp package) (cl-subst :fetcher :host package))))
    (condition-case err
        (if (package-installed-p package)
            (void:log "`%s' already installed." package)
          (void:log "Installing %s package." package)
          (shut-up
            ...
            (if recipe
              ...
              (quelpa recipe))))
      (error
       (void:log "Could not install %s because %S" package err)))))
#+end_src
...
(require 'package)
(package-initialize)
(dolist (package '(leaf leaf-keywords dash anaphora quelpa shut-up))
...
packages. The name choosing was influenced by [[https://github.com/raxod502/radian/blob/develop/emacs/radian.el#L542][radian's dotfiles]].
...
**** unquote                                                          :disabled:
:PROPERTIES:
:ID:       e02675e1-e66d-4250-b84a-0078d6556444
:END:

#+begin_src emacs-lisp
#+end_src

**** unfunctionquote                                                  :disabled:
:PROPERTIES:
:ID:       de215757-dd53-4115-87fb-b8649d717388
:END:
...
**** factorial
:PROPERTIES:
:ID:       01a0195a-d518-4e22-8708-e682b6f462ba
:END:
...
(defmacro fn! (&rest body)
  ...
  (with-symbols! (args)
    `(lambda (,args)
       (-let ((,(void:anaphoric-symbols body) (-list ,args)))
         ,@body))))
#+end_src
...
BODY contains any number of `<NAME>' where NAME refers to the name of the symbol."
  ...
  (let* ((symbols (->> (flatten-list body)
    ...
    `(let* ((,structure-sym ,structure)
            ,@(-map (fn! `(,<sym> (funcall ,fn ,structure-sym ',<name>)))
...
(defmacro defhook! (name args &rest body)
  ...
  (-let* ((docstring (when (stringp (car body)) (pop body)))
          ...
          (local (eq key :local)))
    `(prog1 (defun ,name ,args ,docstring ,@body)
       ,@(-map (lambda (hook) `(add-hook ',hook #',name ,append ,local))
               (-list hooks)))))
#+end_src
...
(defmacro after! (features &rest body)
  ...
  (pcase (car-safe features)
    ('or
     (macroexp-progn
      (-map (fn! `(after! ,<feature> ,@body))
            (cdr features))))
    ('and
     (void:wrap-form (-map (fn! `(after! ,<feature>)) (cdr features))
                     (macroexp-progn body)))
...
(defun kbd::list-data (states maps bindings)
  ...
  (mapcar (-lambda ((map (key def)))
          ...
          (-table-flat #'list maps (-partition 2 bindings))))
#+end_src
...
#+begin_src emacs-lisp
(defmacro leaf:define-keyword! (name args &rest body)
...
(leaf:define-keyword! void-leaf-defer (before :leaf-defer)
...
(leaf:define-keyword! before-call (before :hook)
...
(leaf:define-keyword! after-call (before :hook)
...
#+begin_src emacs-lisp
(leaf:define-keyword! load-incrementally (before :before-call)
  `((append! void:incremental-packages ',leaf--value)
    ,@leaf--body))
#+end_src
...
#+begin_src emacs-lisp
(leaf:set-alias :ensure :void-package)

(leaf:set-normalizer :void-package #'leaf:package-normalizer)

(leaf:define-keyword! void-package (before :package)
  `(,@(when (and leaf--value (car-safe leaf--value))
        `((package:install ',(car leaf--value))))
    ,@leaf--body))

(defun leaf:package-normalizer (leaf--value)
...
#+begin_src emacs-lisp
(leaf:define-keyword! popup (before :mode)
...
#+begin_src emacs-lisp
(leaf:define-keyword! os (before :when)
...
(leaf:define-keyword! alias (before :init)
...
(defhook! exwm:start-in-char-mode-h ()
  ...
  (when (-any-p (fn! (string-prefix-p <prefix> exwm-instance-name))
                (list "emacs"))
...
(defun defsnippet:new-snippet (&rest plist)
  ...
  (let* ((keys '(:key :name :contributor :group :expand-env :binding :type :uuid)))
    (with-plist! plist
      (cl-assert (-all-p (fn! (not (null <it>)))
                         (list <:name> <:mode> <:body>)))
      (alet! (append
              (list "# -*- mode: snippet -*-\n")
              (mapcar (lambda (key)
                        (format "# %s: %s\n" (void:keyword-name key) (plist-get plist key)))
                      (--filter (plist-get plist it) keys))
              (list "# --\n")
...
(use-package! yasnippet
  ...
  :defer-config
  (-each yas-snippet-dirs (fn! (mkdir <dir> t)))
...
(use-package! yasnippet-snippets
  :after yasnippet
  :init
  (awhen! (-first (fn! (s-contains-p "yasnippet-snippets" <path>)) load-path)
...
*** embark
:PROPERTIES:
:ID:       18abb7cd-e98b-437e-a851-922da949d546
:END:
...
**** embark
:PROPERTIES:
:ID:       7d866a13-28b3-4e2b-82de-2126d71c40b6
:END:

#+begin_src emacs-lisp
(use-package! embark
  :ensure (:repo "oantolin/embark" :host github)
  :commands (embark-act embark-act-noexit)
  :init
  (after! selectrum
    (--each '(("C-m" . embark-act) ("C-l" . embark-act-noexit))
      (push it selectrum-minibuffer-bindings))))
#+end_src

**** use a which-key like menu or prompt                              :disabled:
:PROPERTIES:
:ID:       f2dc6833-a90e-4819-b26a-4a8bfae6b340
:END:
...
(setq embark-action-indicator #'embark:which-key-setup
      embark-become-indicator embark-action-indicator)

(defhook! embark:which-key-tear-down ()
  :hook embark-pre-action-hook
...
*** selectrum utility functions
:PROPERTIES:
:ID:       4a744244-f6a7-42c4-8c9a-f7ab73336faf
:END:

**** bindings
:PROPERTIES:
:ID:       87b0f65f-6af2-46ad-8575-a9de3bd63bc3
:END:
...
(general-def
  ...
  [remap org-set-tags-command] #'selectrum/org-set-tag)
#+end_src
...
(defun selectrum/swiper ()
  ...
  (let* ((selectrum-should-sort-p nil)
         ...
         (line-choices (--map-indexed (format "%d\t%s" it-index it) buffer-text-lines))
         ;; Get the matching line.
         (chosen-line
          (completing-read "Jump to matching line: " line-choices
                           nil t nil 'selectrum:swiper-jump-to-line-history))
         ;; Get the corresponding line number.
         (chosen-line-number
          (progn (string-match "\\([[:digit:]]+\\)[[:space:]]\\(?:.\\|\n\\)*" chosen-line)
    ...
    (when (eq major-mode 'org-mode)
      ...
      (when (org:heading-folded-p)
        ...
        (forward-line (1+ (- chosen-line-number (line-number-at-pos (point) t))))))))
#+end_src
...
(defun selectrum/buffer-or-recentf ()
  ...
  (let* ((selectrum-should-sort-p nil)
         (live-buffers
          (--remove (string-match-p (rx bol white) it)
                    (mapcar #'buffer-name (buffer-list))))
         (recentf-buffers
          (--map (cons (propertize (shrink-path-file it) 'face 'font-lock-comment-face) it)
                 recentf-list)))
...
(defun selectrum/org-set-tag ()
  ...
  (let* ((org-use-fast-tag-selection nil)
         ...
         (all-tags (-uniq (flatten-list (org-map-entries (lambda () (org-get-tags (point) t))))))
...
**** choose font
:PROPERTIES:
:ID:       cb09fe43-aff4-4fbe-be4e-12db296298ad
:END:
...
(defun void/set-font ()
  "Set the font."
  (interactive)
  (let ((font-list (-uniq (font-family-list))))
    (awhen! (completing-read "Font: " font-list)
      (set-face-attribute 'default nil :family <font>))))
#+end_src

**** outline
:PROPERTIES:
:ID:       37bf41c1-2b3a-429f-9872-8466b69115fd
:END:
...
WORD is a string with no whitespace characters."
  (-let* ((vowel-regexp (rx (or "a" "e" "i" "o" "u" "y")))
          (letters (-> word
                       ...
                       (s-split "" it t)))
          (no-vowels (-remove (fn! (string-match-p vowel-regexp <string>))
                              letters)))
    (->> no-vowels
         (-uniq)
         (-map #'downcase)
...
If there is no match, return POSSIBLE-NAME."
  (alet! (or (-first (lambda (it) (-some-p (fn! (s-contains-p <name> it t))
...
(defadvice! mu4e:initialize-all-contexts ()
  ...
  :before mu4e
  (require 'password-store)
  (-each (mapcar #'mu4e::account-context (pass:email-list))
    (fn! (cl-pushnew <context> mu4e-contexts))))
#+end_src
...
ELEMENTS is a list of headline elements (see `org:element-headline-elements')."
  (awhen! (-find (fn! (-some-p (apply-partially #'org:element-contains-point-p point)
                               <it>))
                 (->> (cdr elements)
                      (-group-by #'org:element-paragraph-p)
                      (-filter (lambda (it) (org:element-paragraph-p (car it))))))
...
where the point should go."
  ...
  (acond ((-any-p #'org:element-paragraph-p elements)
         ...
         ;; Insert the description after any property drawers.
         (t (cons 'x 'x))
...
(defun org/dwim-edit-description ()
  ...
  (-let* ((elements (org:element-headline-elements))
    ...
    ;; (message "%S" (cons beg end))
    (general-add-hook 'edit-indirect-after-creation-hook #'evil-insert-state)
    (edit-indirect-region beg end t)))
#+end_src
...
(defun org/dwim-edit-source-block ()
  ...
  (if (or (org-in-src-block-p)
          (->> (org:element-headline-elements)
               (--first (eq (org-element-type it) 'src-block))
...
(after! em-term
  (-each '("tmux" "htop" "bash" "zsh" "fish" "vim" "nvim" "ncmpcpp")
    (fn! (add-to-list 'eshell-visual-commands <command>))))
#+end_src
...
(use-feature! autorevert
  ...
  (defhook! auto-revert:visible-buffers-h ()
    ...
    (--each (-uniq (mapcar #'window-buffer (window-list)))
      (with-current-buffer it (auto-revert:buffer-h)))))
#+end_src
...
I'm sure many of us can relate to the feeling of traversing through themes like
TV channels, spending 5 or more minutes trying to find the one that satisfies
our urges for now.
...
(pretty-hydra-define hydra:theme (:title (pretty-hydra-title "Themes" 'faicon "paint-brush")
                                  :color amaranth :quit-key "q")
  ("Emacs"
   (("q" theme-looper-enable-next-theme "next")
    ("D" void/kill-emacs-no-hook "with no hook")
    ("X" theme-looper-enable-random-theme "random"))
   "Processes"
   (("f" theme-looper/add-current-theme-to-favorites "favorites")
    ("f" theme-looper/add-current-theme-to-ignored "ignore"))))
...
*** help
:PROPERTIES:
:ID:       4d314900-312d-42ef-b183-b308b852b17f
:END:

#+begin_src emacs-lisp
#+end_src

** TODO buffer management                                             :disabled:
:PROPERTIES:
:ID:       68cf786d-0b21-40df-9e70-eb546710d61e
:END:

Buffer management is a huge problem in emacs.

*** TODO midnight
:PROPERTIES:
:ID:       68657704-7efc-4d69-9137-8068f21a769c
:END:
:LOGBOOK:
CLOCK: [2020-06-26 Fri 00:24]--[2020-06-26 Fri 00:26] =>  0:02
:END:

There's a package called midnight in emacs that has some cleaning buffer
functionality. Only beef I have with this package is that it is geared towards
being cleaning buffers at midnight, but I'd like them to be cleaned at many
different times. Though there are ways to hack around this.

#+begin_src emacs-lisp
(use-feature! midnight
  :custom
  (clean-buffer-list-delay-special)
  (clean-buffer-list-kill-regexps . (list ))
  )
#+end_src
  ...
  (setq dashboard-init-info
        (format "%d packages loaded in %.2f seconds"
                (cond ((featurep 'straight)
                      ...
                      (t (length package-activated-list)))
...
[[][]] manages the loading of themes at certain time.
...
(define-leader-key!
  :infix "s"
  :keymaps 'org-mode-map
  "h" (list :def #'selectrum/outline :wk "headlines"))
#+end_src
