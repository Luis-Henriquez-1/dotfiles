...
abstractions for manipulating lists. Much of dash's functionality is already
provided by built-in packages. Specifically, I'm referring to the =cl-lib= and
=seq= libraries. The problem though is that the functionality is scattered and.
