...
(defun defsnippet:new-snippet (&rest plist)
  ...
  (let* ((alist (seq-partition plist 2))
    ...
    (with-temp-buffer
      (append
       (list "# -*- mode: snippet -*-\n")
       (seq-map (lambda (it) (format "# %s: %s\n" (void:keyword-name key) it))
                (seq-filter (lambda (it) (member it keys)) alist))
       (list "# --\n")
...
(defun defsnippet:compute-body (body placeholders)
  (alet (seq-map (-lambda ((number name form))
                 ...
                 placeholders)
    (apply #'format body placeholders)))
#+end_src
...
(defsnippet:new-snippet
  ...
  :key "hfn"
  :body "[[helpfn:%s][$1]]$0"
  :placeholders '((1 function (yas-auto-next (yas-choose-value (mapcar #'functionp (void)))))
                  (0 nil (insert "]"))))
#+end_src
...
#+begin_src emacs-lisp
(defsnippet:new-snippet help-variable
  :mode 'org-mode
  :key "hvar"
  :body "[[helpvar:$1][%s]]$0"
  :placeholders '((1 variable (->> (void-list-symbols #'helpful--variable-p)
                               (yas-choose-value)
                               (yas-auto-next)))
                  (0 nil (insert "]"))))
#+end_src
...
(defsnippet:new-snippet
  ...
  :key "oid"
  :body "[[id:%s][#%s]]$0"
  :placeholders '((1 . (thread-last (hash-table-keys org-id-locations)
                        (yas-choose-value)
                        (yas-auto-next)))
                  (2 . (substring (yas-field-value 1) 0 5))))
#+end_src
...
****** defhook!
:PROPERTIES:
:ID:       d6f19fa1-0c22-45a7-bf7c-ca81470ed2b6
:END:

#+begin_src emacs-lisp
(defsnippet:new-snippet
  :name "defhook"
  :body "(defhook! $name $args)"
  :placeholders '((name)
                  (args)
                  (docstring . (-> (rx-to-string `(or ,@(s-split "\s" (yas-field-value 2) t)))
                                (replace-regexp-in-string #'upcase yas-text)
                                (ignore-errors)))))
#+end_src
